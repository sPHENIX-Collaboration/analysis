...
int main(int argc, char *argv[])
{
    ...
    float beamspot_x = 0.544991;
    float beamspot_y = -0.00600619;
    ...
    if (IsData)
    {
        ...
        int runnumber = 45199;
        infilename = "/sphenix/user/hjheng/sPHENIXRepo/mvtx_standalone_cluster/macros/outputClusters_000"+ TString::Format("%d", runnumber) + ".root";
        outdirname = "/sphenix/user/hjheng/sPHENIXRepo/analysis/MVTXTracklet/MVTX_Run2024_000" + TString::Format("%d", runnumber);
        system("mkdir -p " + outdirname);
        outfilename = outdirname + "/Minitree_TrackletRecoVtx_000" + TString::Format("%d", runnumber) + ".root";
        demoplotname = "./plot/RecoPV_demo/RecoPV_Data/Run000" + TString::Format("%d", runnumber);
    ...
    t->SetBranchAddress("globalX", &ClusX);
    t->SetBranchAddress("globalY", &ClusY);
    t->SetBranchAddress("globalZ", &ClusZ);
    ...
    // for (Long64_t ev = 0; ev < t->GetEntriesFast(); ev++)
    {
        ...
        for (size_t ihit = 0; ihit < ClusLayer->size(); ihit++)
        {
            ...
            {
                Hit *hit = new Hit(cpos[0], cpos[1], cpos[2], beamspot_x, beamspot_y, 0., 0);
            ...
            {
                Hit *hit = new Hit(cpos[0], cpos[1], cpos[2], beamspot_x, beamspot_y, 0., 1);
        ...
        {
            Draw_RecoVtxZTklZRho(hM_clusterzrho_halves1, hM_clusterzrho_halves2, tl_zrho_halves1, tl_zrho_halves2, PV_halves1[2], PV_halves2[2], PV_z, IsData, Form("%s/Evt%d_RecoPV_ZRho", demoplotname.Data(), event));
            Draw_RecoVtxTklXY(hM_clusterxy_halves1, hM_clusterxy_halves2, tl_xy_halves1, tl_xy_halves2, PV_halves1[0], PV_halves2[0], PV_x, PV_halves1[1], PV_halves2[1], PV_y, dZCutbin, IsData, Form("%s/Evt%d_RecoPV_XY", demoplotname.Data(), event));
        ...
        Draw_1DHistVtxDemo(hM_diffExptzPVz_halves1, "v_{Z}^{Candidate} - PV_{Z}^{Estimate} [cm]", "cm", 510, IsData, true, "2023-08-30", Form("%s/Evt%d_diffExptzPVz", demoplotname.Data(), event));
        hM_diffExptxyPVxy_halves1->Add(hM_diffExptxyPVxy_halves2);
        Draw_2DHistVtxDemo(hM_diffExptxyPVxy_halves1, "v_{X}^{Candidate} - PV_{X}^{Estimate} [cm]", "v_{Y}^{Candidate} - PV_{Y}^{Estimate} [cm]", 510, IsData, true, "2023-08-30", Form("%s/vt%d_diffExptxyPVxy", demoplotname.Data(), event));

        hM_VtxCandZ_halves1->Add(hM_VtxCandZ_halves2);
        Draw_1DHistVtxDemo(hM_VtxCandZ_halves1, "v_{Z}^{Candidate} [cm]", "cm", 510, IsData, true, "2023-08-30", Form("%s/Evt%d_VtxCandZall", demoplotname.Data(), event));
        hM_VtxCandXY_halves1->Add(hM_VtxCandXY_halves2);
        Draw_2DHistVtxDemo(hM_VtxCandXY_halves1, "v_{X}^{Candidate} [cm]", "v_{Y}^{Candidate} [cm]", 510, IsData, true, "2023-08-30", Form("%s/Evt%d_VtxCandXYcluster", demoplotname.Data(), event));

        hM_DCAxy_VtxClus_halves1->Add(hM_DCAxy_VtxClus_halves2);
        Draw_1DHistVtxDemo(hM_DCAxy_VtxClus_halves1, "DCA_{xy} [cm]", "cm", 505, IsData, false, "", Form("%s/Evt%d_DCAxy", demoplotname.Data(), event));
        hM_DCAxy_DCAz_VtxClus_halves1->Add(hM_DCAxy_DCAz_VtxClus_halves2);
        Draw_2DHistVtxDemo(hM_DCAxy_DCAz_VtxClus_halves1, "DCA_{xy} [cm]", "DCA_{z} [cm]", 505, IsData, false, "", Form("%s/Evt%d_DCAxyDCAz", demoplotname.Data(), event));
        hM_DCAx_DCAy_VtxClus_halves1->Add(hM_DCAx_DCAy_VtxClus_halves2);
        Draw_2DHistVtxDemo(hM_DCAx_DCAy_VtxClus_halves1, "DCA_{x} [cm]", "DCA_{y} [cm]", 505, IsData, false, "", Form("%s/Evt%d_DCAxDCAy", demoplotname.Data(), event));
