...
void JetIndicesMatcher::add_truth(vector<float>& eta, vector<float>& phi, vector<float>& pt) {
    vector<array<float,3>> jets;
    for (unsigned int i=0; i<eta.size(); ++i) {
        jets.push_back({pt[i],eta[i],phi[i]});
    }
...
void JetIndicesMatcher::add_reco(vector<float>& eta, vector<float>& phi, vector<float>& pt) {
    ...
    /* cout << " First: " << endl; */
    /* for (auto jet : jets) cout << jet[0] <<":"<<jet[1]<<":"<<jet[2] << endl; */
    std::sort(jets.rbegin(), jets.rend());
    /* cout << " second: " << endl; */
    for (auto jet : jets) {
      /* cout << jet[0] <<":"<<jet[1]<<":"<<jet[2] << endl; */
        ...
        for (unsigned int R=0;R<eta_reco.size();++R) {
          if (pt_reco[R]<min_pT_reco) continue;
            ...
            if (R2_comp<=R2) {
                found_match = true;
                is_matched[R] = true;
                indices_matched.push_back({T,R});

                /* cout << " ADDED MATCH( " << T << ":"<<R<<")" << endl; */
                break;
            }
        }
        if (!found_match) indices_miss.push_back(T);
    }
    for (unsigned int R=0;R<eta_reco.size();++R) {
        if (pt_reco[R]<min_pT_reco) continue;
        if (!is_matched[R]) indices_fake.push_back(R);
    }
