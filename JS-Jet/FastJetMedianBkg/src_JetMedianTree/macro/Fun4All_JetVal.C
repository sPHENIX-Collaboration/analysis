...
void Fun4All_JetVal(
    const char *filelisttruth = "",
    const char *filelistcalo = "",
		const char *filelistglobal = "",
    const char *filelistbbc = "",
		const char *outname = "output_test.root")
{
  ...
  Fun4AllInputManager *in_bbc = new Fun4AllDstInputManager("BBC_info");
  in_bbc->AddListFile(filelistbbc);
  se->registerInputManager(in_bbc);
