...
parser.add_argument('-o', '--condorDir', default="./", help='Condor Output Folder')
...
dstSets = ['DST_TRACKS']
dstSets.append('DST_CALO_CLUSTER')
dstSets.append('DST_TRUTH')
#dstSets.append('DST_TRUTH_JET')
#added run 8
#dstSets.append('DST_TRACKSEEDS')
dstSets.append('DST_GLOBAL')


"""
if not args.nopileup: 
    stSets.append('DST_VERTEX')
if args.truth:
    #args.g4hit = False
    dstSets.append('DST_TRUTH')
    if not args.nopileup: 
...
def makeCondorJob():
    ...
    condorFile.write("Arguments          = \"$(condorDir) {}\"\n".format(' '.join(listFileGeneric)))
