...
R__LOAD_LIBRARY(/sphenix/u/damian0227/install/lib/libJetKinematicCheck.so)
...
void Fun4All_JetVal(
                     const char *filelistcalo = "/sphenix/u/damian0227/analysis/JS-Jet/JetAnalysis/macro/dst_calo_cluster.list",
		     const char *hits_mbd = "/sphenix/u/damian0227/analysis/JS-Jet/JetAnalysis/macro/dst_bbc_g4hit.list",
                     const char *outname = "output.root")
{
  ...
  JetKinematicCheck *myJetKC = new JetKinematicCheck("AntiKt_Tower_r02_Sub1","AntiKt_Tower_r04_Sub1", outname);

  myJetKC->setPtRange(10, 100);
  myJetKC->setEtaRange(-1.1, 1.1);
  se->registerSubsystem(myJetKC);
  
  Fun4AllInputManager *intrue = new Fun4AllDstInputManager("DSTbbc");
  intrue->AddListFile(hits_mbd,1);
