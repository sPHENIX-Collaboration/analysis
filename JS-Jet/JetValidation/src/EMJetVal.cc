...
int EMJetVal::retrieveEvent(vector<PseudoJet>& event) {
  ...
  // Access the necessary data and populate the event vector
  // You'll need to modify this part to match your actual data structure
  // Assuming you have some data members like m_eta, m_phi, m_e, and m_pt
  // and assuming these vectors have the event data
  for (int i = 0; i < m_eta.size(); ++i) {
    ...
    // Create a PseudoJet and add it to the event vector
    PseudoJet particle(pt * cos(phi), pt * sin(phi), pt * sinh(eta), e);
      ...
      if(m_doUnsubJet)
	{
	  ...
	  for (Jet::ConstIter comp = jet->begin_comp(); comp != jet->end_comp(); ++comp)
	      ...
	      else if ((*comp).first == 14 || (*comp).first == 29)
		{
		  ...
		  int numEvents = 1000; // Initialize numEvents with the appropriate value
		  for (int eventIndex = 0; eventIndex < numEvents; ++eventIndex) {
		    // Create a vector to store the event data
		    vector<PseudoJet> event;

		    EMJetVal emJetVal; // Create an instance of the EMJetVal class
		    // Retrieve the event data using the retrieveEvent function
		    int retrieveStatus = emJetVal.retrieveEvent(event);

		    if (retrieveStatus != 0) {
		      cerr << "Error retrieving event data for event " << eventIndex << endl;
		      // Handle the error, e.g., by skipping this event or exiting the loop
		      continue;
		    }
