	...
	if(m_doUnsubJet)
	  {
		...
		if ((*comp).first == 15 ||  (*comp).first == 30)
		...
		else if ((*comp).first == 14 || (*comp).first == 29)
		  {
		    ...
		    vector<PseudJet> sortedJets_R04;
			...
			if (UE[i].isFinal() && UE[i].isVisible() && abs(UE[i].tower_eta()) < etaMax){
			  particles.push_back(PseudoJet( UE[i].totalPx(), UE[i].totalPy(), UE[i].totalPz(), UE[i].totalE() ) );
			  if(UE[i].charge() != 0)
			    mult++; //! mult++ = mult + 1;
