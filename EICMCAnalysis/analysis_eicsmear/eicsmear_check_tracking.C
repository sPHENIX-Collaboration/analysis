int
eicsmear_check_tracking( TString filename_mc,
			 TString filename_mc_smeared = "")
{
  ...
  TCut cut_base("particles.KS==1 && Smeared.particles.p > 0");
  ...
  h2tmp->GetXaxis()->SetTitle("Momentum (GeV/c)");
  h2tmp->GetYaxis()->SetTitle("(#Delta p)/p");
  ...
  for ( int i = 0; i < etas.size(); i++ )
    {
      ...
      h2tmp->Reset();
      tree->Draw("(Smeared.particles.p-particles.p)/particles.p:particles.p >> h2tmp",cut_base && cutx,"");
      h2tmp->FitSlicesY();
