...
eic_sphenix_dis_fillbins()
{
  ...
  /* print bin centers for y bins */
  for ( int bin_y = 1; bin_y <= hxQ2->GetNbinsY(); bin_y++ )
    {

      fprintf( stdout, "Q2 = %.2e\n",
	       TMath::Power(10, 0.5 * ( ( TMath::Log10( hxQ2->GetYaxis()->GetBinLowEdge(bin_y) ) )
  ...
  for ( unsigned ievent = 0; ievent < tree->GetEntries(); ievent++ )
    {
      ...
      if ( event->ScatteredLepton()->GetEta() < -4 ||
	   event->ScatteredLepton()->GetEta() >  4 )
	continue;
