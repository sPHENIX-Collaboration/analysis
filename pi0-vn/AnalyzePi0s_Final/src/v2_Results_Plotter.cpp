...
void plotting_Results_andPHENIXoverlay(const Data& data1) {
    ...
    */
    auto create_sPHENIX_Results = [&data1](TCanvas* canvas, TGraphErrors* graph1, TGraphErrors* graph2,  const std::string& centrality) {
        ...
        graph1->SetMinimum(minYaxis_correctedComparisons); // Set the minimum y value
        graph1->SetMaximum(maxYaxis_correctedComparisons); // Set the maximum y value
        graph1->GetXaxis()->SetLimits(2.0, 5.0);
        DrawZeroLine(canvas);
        Create_sPHENIX_legend(canvas, 0.14, 0.19, 0.34, 0.39, centrality.c_str(), 0.045);
    };
