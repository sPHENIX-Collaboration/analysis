...
void ComboFill() {
    ...
    std::vector<float> energyCuts = {1};
    

    std::vector<float> asymmetryCuts = {.15 ,.2};

    std::vector<float> deltaRCuts = {.07, 0.08, 0.085};
    
    std::vector<float> chiCuts = {4, 5};
        ...
        for (float cut_asymmetry : asymmetryCuts) {
            for (float cut_delta_R : deltaRCuts) {
                for (float cut_clus_chi : chiCuts) {
                    ...
                    TH1F *hPi0Mass[12];
                    for (int i = 0; i < 12; ++i) {
                        std::string histName = "hPi0Mass_E" + formatFloatForFilename(cut_clus_E) +
                                               "_Asym" + formatFloatForFilename(cut_asymmetry) +
                                               "_Delr" + formatFloatForFilename(cut_delta_R) +
                                               "_Chi" + formatFloatForFilename(cut_clus_chi) +
                                               "_" + std::to_string(i);
