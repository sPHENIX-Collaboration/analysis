...
data = pd.read_csv('/Users/patsfan753/Desktop/vN_AnalysisFinal/data/PlotByPlotOutput_FromFits/p011/PlotByPlotOutput_p011.csv')

# Dictionary to maintain the visibility state of different centrality categories in the plot
centrality_visibility = {"40-60%": True, "20-40%": True, "0-20%": True}

# List of labels corresponding to different centrality categories
labels = ["40-60%", "20-40%", "0-20%"]

centrality_categories = {
    '40-60%': [0, 1, 2, 3, 4, 5],
    '20-40%': [6, 7, 8, 9, 10, 11],
    '0-20%': [12, 13, 14, 15, 16, 17]
}
pt_categories = {
...
def plot_SignalYield(filtered_data, clear_plot):
    ...
    for i, (color, marker) in enumerate(zip(colors, markers)):
        ...
        base_jitter_offset = (i - (len(centrality_categories)/2 - 0.5)) * 0.05
        ...
        indices = centrality_categories[category_label]
        ...
        for j, (x_val, y_val, y_err, S_B_ratio, numEntry) in enumerate(zip(x_vals, y_vals, y_errs, S_B_ratios, numEntries)):
            index = indices[j]  # Directly use the index from the updated list
...
def plot_index_data(index_str, history_dict):
    try:
        ...
        ax.set_title(f'{current_plot_type} {centrality_label}, {pt_label}', fontsize=16)
        ...
        # Place the legend outside the plot at the bottom right
        leg = ax.legend(legend_handles, legend_str_list, loc='upper left', bbox_to_anchor=(.7, 0.9), fontsize='medium', title='Cut Combinations', handlelength=0, handletextpad=0)
...
def summarize_centrality(root):
    global current_plot_type
    title = "Summarize Data For Specific Centrality"
    
    centrality_window = tk.Toplevel(root)
    centrality_window.title(title)

    tk.Label(centrality_window, text="Enter Centrality (e.g., '20-40%'):").pack(side=tk.LEFT)
    centrality_entry = tk.Entry(centrality_window)
    centrality_entry.pack(side=tk.LEFT)
    
    # This button will trigger the plotting function specifically designed for centrality summaries
    submit_button = ttk.Button(centrality_window, text="Submit",
                               command=lambda: plot_centrality_data(centrality_entry.get()))
    submit_button.pack(side=tk.LEFT)

def plot_centrality_data(centrality_str):
    if centrality_str not in centrality_categories:
        messagebox.showerror("Error", f"No data found for centrality '{centrality_str}'")
        return

    # Fetch data for the specified centrality
    centrality_data = [data for data in plot_SBratio_history.values() if data['centrality'] == centrality_str]
    ...
    # Aggregate data by cut combinations
    cut_combinations = {}
    for data_point in centrality_data:
        cuts = tuple(sorted(data_point['cuts'].items()))
        if cuts not in cut_combinations:
            cut_combinations[cuts] = []
        cut_combinations[cuts].append(data_point)

    # Calculate weighted averages and errors
    x_vals, y_vals, y_errs, labels = [], [], [], []
    for i, (cuts, data_points) in enumerate(cut_combinations.items()):
        weighted_sum, total_weight, squared_errors_sum = 0, 0, 0
        print(f"Processing cut combination: {dict(cuts)}")
        for data_point in data_points:
            weight = 6 - centrality_categories[centrality_str].index(data_point['csv_index'])
            sb_val = data_point['sb_val']
            sb_err = data_point['sb_err']
            print(f"\tS/B: {sb_val}, Error: {sb_err}, Weight: {weight}")
            weighted_sum += sb_val * weight
            total_weight += weight
            squared_errors_sum += (sb_err * weight) ** 2

        weighted_average = weighted_sum / total_weight if total_weight else 0
        weighted_error = np.sqrt(squared_errors_sum) / total_weight if total_weight else 0
        print(f"\tWeighted Average S/B: {weighted_average}, Weighted Error: {weighted_error}\n")

        x_vals.append(i)
        y_vals.append(weighted_average)
        y_errs.append(weighted_error)
        labels.append(', '.join(f"{k}: {v}" for k, v in dict(cuts).items()))

    # Plot with error bars
    ax.errorbar(x_vals, y_vals, yerr=y_errs, fmt='o', color='blue', ecolor='lightblue', elinewidth=3, capsize=0, alpha=0.7)
    ax.set_xticks(x_vals)
    ax.set_xticklabels(labels, rotation=45, ha="right", fontweight='bold', fontsize=10)  # Adjust fontsize as needed
    ax.set_title(f'Weighted Average S/B Ratio for {centrality_str} Centrality', fontsize=18)
    ax.set_ylabel('Weighted Average S/B Ratio', fontsize=16)
    ax.set_xlabel('Cut Combinations', fontsize=16)
    
    # Text for the x-axis labels description
    label_text = r"$\bf{x-axis\ labels:}$ A → Asymmetry, C → $\chi^2$, D → $\Delta R$, E → Energy"
    
    # Place the text in the top right corner of the plot
    ax.text(0.98, 0.12, label_text, transform=ax.transAxes, fontsize=12, verticalalignment='top', horizontalalignment='right', bbox=dict(facecolor='white', alpha=0, edgecolor='white'))
    ...
    def save_figure():
        ...
        if file_path:
            file_format = 'PDF' if file_path.endswith('.pdf') else 'PNG'
            fig.savefig(file_path, dpi=300, format=file_format.lower(), bbox_inches='tight')
...
class InfoDialog(simpledialog.Dialog):
    ...
    def open_histogram(index, cuts):
        ...
        histogram_path = "/Users/patsfan753/Desktop/vN_AnalysisFinal/plotOutput/InvMassPlots"
...
def create_main_application(root):
 ...
 # Add a button to summarize centrality data
    summarize_centrality_button = ttk.Button(left_frame, text="Summarize Centrality", command=lambda: summarize_centrality(root))
    summarize_centrality_button.grid(row=9, column=0, columnspan=2, sticky='ew', pady=(10, 0))  # Adjust grid row as needed
    ...
    generate_table_button.grid(row=10, column=0, columnspan=2, sticky='ew', pady=(10, 0))
    ...
    if current_plot_type == "SignalYield":
        ...
        transformation_button.grid(row=11, column=0, columnspan=2, sticky='ew', pady=(10, 0))
        centrality_frame_row = 12  # We position the centrality frame below the transformation button
        padyVal = 90
    else:
        centrality_frame_row = 11  # There is no transformation button, so centrality frame moves up
        padyVal = 100
