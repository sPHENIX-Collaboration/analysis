...
pi0Ana.add_argument('-s2', '--isSim', type=int, default=0, help='Type Simulation. Default: False')
    ...
    with open(ntp_list) as file:
        for line in file:
            ...
            with open(f'{output_dir}/{run}/genPi0Ana.sub', mode="w") as file2:
                ...
                if(do_vn_calc):
                    file2.write(f'arguments      = {output_dir}/{os.path.basename(executable)} $(input_ntp) {cuts} {isSim} {z} output/test-$(Process).root {anaType} {do_vn_calc} {fitStats} {Q_vec_corr} {samples} {cut_num} {sigma}\n')
                else:
                    file2.write(f'arguments      = {output_dir}/{os.path.basename(executable)} $(input_ntp) {cuts} {isSim} {z} output/test-$(Process).root {anaType}\n')
