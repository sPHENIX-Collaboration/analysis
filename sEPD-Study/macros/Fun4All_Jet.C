...
#include <sepdvalidation/JetValidation.h>

#include <CDBUtils.C>

R__LOAD_LIBRARY(libsEPDValidation.so)

void Fun4All_Jet(const std::string &dst_jetcalo,
                 const std::string &dst_jet,
                 ...
                 const std::string &dbtag = "newcdbtag")
{
  ...
  std::cout << "input 1: " << dst_jetcalo << std::endl;
  std::cout << "input 2: " << dst_jet << std::endl;
  ...
  std::pair<Int_t, Int_t> runseg = Fun4AllUtils::GetRunSegment(dst_jetcalo);
  ...
  In->AddFile(dst_jetcalo);
  se->registerInputManager(In.get());

  std::unique_ptr<Fun4AllInputManager> In2 = std::make_unique<Fun4AllDstInputManager>("in2");
  In2->AddFile(dst_jet);
  se->registerInputManager(In2.get());
...
int main(int argc, const char* const argv[])
{
  ...
  {
    std::cerr << "usage: " << args[0] << " <input_DST_jetcalo> <input_DST_jet> [output] [nEvents] [dbtag]" << std::endl;
    std::cerr << "  input_DST_jetcalo: path to input jetcalo dst" << std::endl;
    std::cerr << "  input_DST_jet: path to input jet dst" << std::endl;
  ...
  const std::string& input_dst_jetcalo= args[1];
  const std::string& input_dst_jet= args[2];
  ...
  Fun4All_Jet(input_dst_jetcalo, input_dst_jet, output, nEvents, dbtag);
