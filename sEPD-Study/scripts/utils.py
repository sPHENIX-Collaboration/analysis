...
trigger_qa = subparser.add_parser('trigger_qa', help='Create condor submission directory.')

trigger_qa.add_argument('-i'
                    , '--input-list', type=str
                    , required=True
                    , help='Input DST List.')

trigger_qa.add_argument('-i2'
                    , '--dbtag', type=str
                    , default='newcdbtag'
                    , help='CDB Tag. Default: newcdbtag')

trigger_qa.add_argument('-o'
                    , '--output-dir', type=str
                    , default='scratch/test'
                    , help='Project Directory. Default: scratch/test')

trigger_qa.add_argument('-n'
                    , '--events', type=int
                    , default=0
                    , help='Number of events to analyze. Default: All.')

trigger_qa.add_argument('-n2'
                    , '--dst-per-job', type=int
                    , default=2
                    , help='Number of DSTs to analyze per job. Default: 2.')

trigger_qa.add_argument('-s'
                    , '--memory', type=float
                    , default=1
                    , help='Memory (units of GB) to request per condor submission. Default: 1 GB.')

trigger_qa.add_argument('-l'
                    , '--condor-log-dir', type=str
                    , default='/tmp/anarde/dump'
                    , help='Condor Log Directory. Default: /tmp/anarde/dump')

trigger_qa.add_argument('-f'
                    , '--f4a-macro', type=str
                    , default='macros/Fun4All_TriggerQA.C'
                    , help='Fun4All Macro. Default: macros/Fun4All_TriggerQA.C')

trigger_qa.add_argument('-f2'
                    , '--src-dir', type=str
                    , default='src'
                    , help='Source Files Directory. Default: src')

trigger_qa.add_argument('-f3'
                    , '--condor-script', type=str
                    , default='scripts/genTriggerQA.sh'
                    , help='Condor Script. Default: scripts/genTriggerQA.sh')

trigger_qa.add_argument('-f4'
                    , '--common-errors', type=str
                    , default='files/common-errors.txt'
                    , help='Common Errors. Default: files/common-errors.txt')

def create_trigger_qa_jobs():
    """
    Create Fun4All Jobs
    """
    ...
    for f in [input_list, condor_script, f4a_macro, common_errors]:
        if not f.is_file():
...
f4a.add_argument('-s'
                    , '--memory', type=float
                    , default=1.5
                    , help='Memory (units of GB) to request per condor submission. Default: 1.5 GB.')
...
def create_f4a_jobs():
    ...
    submit_file_content = textwrap.dedent(f"""\
        executable     = {condor_script.name}
        arguments      = {f4a_macro} $(input_dst) $(input_calib) test-$(ClusterId)-$(Process).root tree-$(ClusterId)-$(Process).root {events} {dbtag} {output_dir}/output
        log            = {condor_log_dir}/job-$(ClusterId)-$(Process).log
        output         = stdout/job-$(ClusterId)-$(Process).out
        error          = error/job-$(ClusterId)-$(Process).err
        request_memory = {condor_memory}GB
...
f4a_mc = subparser.add_parser('f4a_mc', help='Create condor submission directory.')

f4a_mc.add_argument('-i'
                    , '--input-list', type=str
                    , required=True
                    , help='Input DST List.')
...
f4a_mc.add_argument('-f3'
                    , '--condor-script', type=str
                    , default='scripts/genFun4All_MC.sh'
                    , help='Condor Script. Default: scripts/genFun4All_MC.sh')

f4a_mc.add_argument('-f4'
                    , '--common-errors', type=str
                    , default='files/common-errors.txt'
                    , help='Common Errors. Default: files/common-errors.txt')
...
calo_qa = subparser.add_parser('calo_qa', help='Create condor submission directory.')

calo_qa.add_argument('-i'
                    , '--input-list', type=str
                    , required=True
                    , help='Input DST List.')

calo_qa.add_argument('-i2'
                    , '--dbtag', type=str
                    , default='newcdbtag'
                    , help='CDB Tag. Default: newcdbtag')

calo_qa.add_argument('-o'
                    , '--output-dir', type=str
                    , default='scratch/test'
                    , help='Project Directory. Default: scratch/test')

calo_qa.add_argument('-n'
                    , '--events', type=int
                    , default=0
                    , help='Number of events to analyze. Default: All.')

calo_qa.add_argument('-s'
                    , '--memory', type=float
                    , default=2
                    , help='Memory (units of GB) to request per condor submission. Default: 2 GB.')

calo_qa.add_argument('-l'
                    , '--condor-log-dir', type=str
                    , default='/tmp/anarde/dump'
                    , help='Condor Log Directory. Default: /tmp/anarde/dump')

calo_qa.add_argument('-f'
                    , '--f4a-macro', type=str
                    , default='macros/Fun4All_CaloQA.C'
                    , help='Fun4All Macro. Default: macros/Fun4All_CaloQA.C')

calo_qa.add_argument('-f2'
                    , '--src-dir', type=str
                    , default='src'
                    , help='Source Files Directory. Default: src')

calo_qa.add_argument('-f3'
                    , '--condor-script', type=str
                    , default='scripts/genFun4All_CaloQA.sh'
                    , help='Condor Script. Default: scripts/genFun4All_CaloQA.sh')

calo_qa.add_argument('-f4'
                    , '--common-errors', type=str
                    , default='files/common-errors.txt'
                    , help='Common Errors. Default: files/common-errors.txt')

calo_qa.add_argument('-b'
                    , '--f4a-bin', type=str
                    , default='bin/Fun4All_CaloQA'
                    , help='Fun4All Bin. Default: bin/Fun4All_CaloQA')

def create_calo_qa_jobs():
    """
    Create Fun4All Jobs
    """
    input_list = Path(args.input_list).resolve()
    ...
    # Ensure that files exists
    for f in [input_list, condor_script, f4a_bin, f4a_macro, common_errors]:
        if not f.is_file():
            logger.critical(f'File: {f} does not exist!')
            sys.exit()
    ...
    submit_file_content = textwrap.dedent(f"""\
        executable     = {condor_script.name}
        arguments      = {f4a_bin} $(input_dst) test-$(ClusterId)-$(Process).root {events} {dbtag} {output_dir}/output
        log            = {condor_log_dir}/job-$(ClusterId)-$(Process).log
        output         = stdout/job-$(ClusterId)-$(Process).out
        error          = error/job-$(ClusterId)-$(Process).err
        request_memory = {condor_memory}GB
...
centrality_qa = subparser.add_parser('centrality_qa', help='Create condor submission directory.')

centrality_qa.add_argument('-i'
                    , '--input-list', type=str
                    , required=True
                    , help='Input DST List.')

centrality_qa.add_argument('-i2'
                    , '--dbtag', type=str
                    , default='newcdbtag'
                    , help='CDB Tag. Default: newcdbtag')

centrality_qa.add_argument('-o'
                    , '--output-dir', type=str
                    , default='scratch/test'
                    , help='Project Directory. Default: scratch/test')

centrality_qa.add_argument('-n'
                    , '--events', type=int
                    , default=0
                    , help='Number of events to analyze. Default: All.')

centrality_qa.add_argument('-s'
                    , '--memory', type=float
                    , default=2
                    , help='Memory (units of GB) to request per condor submission. Default: 2 GB.')

centrality_qa.add_argument('-l'
                    , '--condor-log-dir', type=str
                    , default='/tmp/anarde/dump'
                    , help='Condor Log Directory. Default: /tmp/anarde/dump')

centrality_qa.add_argument('-f'
                    , '--f4a-macro', type=str
                    , default='macros/Fun4All_CentralityQA.C'
                    , help='Fun4All Macro. Default: macros/Fun4All_CentralityQA.C')

centrality_qa.add_argument('-f2'
                    , '--src-dir', type=str
                    , default='src'
                    , help='Source Files Directory. Default: src')

centrality_qa.add_argument('-f3'
                    , '--condor-script', type=str
                    , default='scripts/genFun4All_CentralityQA.sh'
                    , help='Condor Script. Default: scripts/genFun4All_CentralityQA.sh')

centrality_qa.add_argument('-f4'
                    , '--common-errors', type=str
                    , default='files/common-errors.txt'
                    , help='Common Errors. Default: files/common-errors.txt')

centrality_qa.add_argument('-b'
                    , '--f4a-bin', type=str
                    , default='bin/Fun4All_CentralityQA'
                    , help='Fun4All Bin. Default: bin/Fun4All_CentralityQA')

def create_centrality_qa_jobs():
    """
    Create Fun4All Jobs
    """
    input_list = Path(args.input_list).resolve()
...
if __name__ == "__main__":
    ...
    if args.command == 'centrality_qa':
        create_centrality_qa_jobs()

    if args.command == 'trigger_qa':
        create_trigger_qa_jobs()
