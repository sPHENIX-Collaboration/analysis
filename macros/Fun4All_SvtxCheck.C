...
int Fun4All_SvtxCheck(const int jobNum = 0,
		      const int nEvents = 5
		      ) {
  ...
  const char* outputFile = Form("out/histos_%i_%05i.root",time.GetDate(),jobNum);
  //const char* completeFlag = Form("out/histos_%i_%05i.done",time.GetDate(),jobNum);
  ...
  se->registerSubsystem(new SvtxSimPerformanceCheckReco());
  ...
  //gSystem->Command(Form("touch %s",completeFlag))
}
