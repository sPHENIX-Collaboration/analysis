...
// note : p1 : inner layer
// note : p2 : outer layer
double Get_eta(vector<double>p0, vector<double>p1, vector<double>p2)
{
    ...
    else 
    {
        ...
        return  -1 * TMath::Log( fabs( tan( atan2(ax_b.first, (ax_b.first > 0) ? 1. : -1. ) / 2 ) ) );
...
void dNdeta(/*pair<double,double>beam_origin*/)
{
    ...
    system(Form("mkdir %s/folder_%s/good_track",mother_folder_directory.c_str(),file_name.c_str()));
    ...
    double mean_zvtx = -210.67; // note : unit : mm

    bool full_event = false;
    long long used_event = 10000;

    double dNdeta_upper_range = 1.5;
    ...
    for (int event_i = 0; event_i < N_event; event_i++)
    {
            ...
            for ( int outer_i = 0; outer_i < temp_sPH_outer_nocolumn_vec.size(); outer_i++ )
            {
                ...
                if (fabs(temp_sPH_inner_nocolumn_vec[inner_i].phi - temp_sPH_outer_nocolumn_vec[outer_i].phi) < phi_diff_cut)
                {
                    ...
                    if (DCA_info_vec[0] < DCA_cut){
                        ...
                        good_comb_xy_vec.push_back({
                            temp_sPH_inner_nocolumn_vec[inner_i].x, temp_sPH_inner_nocolumn_vec[inner_i].y,
                            temp_sPH_outer_nocolumn_vec[outer_i].x, temp_sPH_outer_nocolumn_vec[outer_i].y
                        });

                        good_comb_phi_vec.push_back({
                            temp_sPH_inner_nocolumn_vec[inner_i].phi,
                            temp_sPH_outer_nocolumn_vec[outer_i].phi
                        });
        ...
        if (temp_event_zvtx_vec.size() > zvtx_cal_require)
        {
            ...
            zvtx_evt_width -> Fill(fabs(temp_event_zvtx_info[1] - temp_event_zvtx_info[2]));
            
            // cout<<"test0 : "<<N_hits<<" Ncluster : "<<(temp_sPH_inner_nocolumn_vec.size() + temp_sPH_outer_nocolumn_vec.size())<<" N good track : "<<good_comb_rz_vec.size()<<" temp_event_zvtx_vec size : "<<temp_event_zvtx_vec.size()<<endl;
            ...
            if ( fabs(temp_event_zvtx_info[0] - mean_zvtx) < 25 && fabs(temp_event_zvtx_info[1] - temp_event_zvtx_info[2]) < 60 ) { // note : the measured z vetex resolution < 70 mm
                
                N_good_event += 1;

                cout<<"test : "<<N_hits<<" Ncluster : "<<(temp_sPH_inner_nocolumn_vec.size() + temp_sPH_outer_nocolumn_vec.size())<<" N good track : "<<good_comb_rz_vec.size()<<" temp_event_zvtx_vec size : "<<temp_event_zvtx_vec.size()<<endl;

                c4 -> cd(); 
                Good_track_space -> Draw();

                // note : for the quick dN/deta calculation
                for (int track_i = 0; track_i < good_comb_rz_vec.size(); track_i++){
                    // -1 * TMath::Log( tan( (30/(180./M_PI)) / 2 ) )

                    if (good_track_rz_vec[track_i][2] > temp_event_zvtx_info[1] && good_track_rz_vec[track_i][2] < temp_event_zvtx_info[2]){
                        
                        double track_eta = Get_eta(
                            { sqrt(pow(beam_origin.first,2)+pow(beam_origin.second,2)), temp_event_zvtx_info[0], fabs(temp_event_zvtx_info[1] - temp_event_zvtx_info[2])/2. },
                            { good_comb_rz_vec[track_i][0], good_comb_rz_vec[track_i][1] }, // note : inner layer, r & z
                            { good_comb_rz_vec[track_i][2], good_comb_rz_vec[track_i][3] }  // note : outer layer, r & z 
                        );

                        double track_eta_inner = Get_eta_2P(
                            { sqrt(pow(beam_origin.first,2)+pow(beam_origin.second,2)), temp_event_zvtx_info[0], fabs(temp_event_zvtx_info[1] - temp_event_zvtx_info[2])/2. },
                            { good_comb_rz_vec[track_i][0], good_comb_rz_vec[track_i][1] } // note : inner layer, r & z
                        );

                        double track_eta_outer = Get_eta_2P(
                            { sqrt(pow(beam_origin.first,2)+pow(beam_origin.second,2)), temp_event_zvtx_info[0], fabs(temp_event_zvtx_info[1] - temp_event_zvtx_info[2])/2. },
                            { good_comb_rz_vec[track_i][2], good_comb_rz_vec[track_i][3] }  // note : outer layer, r & z 
                        );

                        good_tracklet_rz.push_back({
                            sqrt(pow(beam_origin.first,2)+pow(beam_origin.second,2)), temp_event_zvtx_info[0], // note : origin r, z
                            good_comb_rz_vec[track_i][0], good_comb_rz_vec[track_i][1] // note : inner r, z
                        });
                        
                        // note : inner z (nZ B)
                        if      (good_comb_rz_vec[track_i][1] < -130 ) { Good_inner_outer_pos_xy_nzB -> Fill(good_comb_xy_vec[track_i][0],good_comb_xy_vec[track_i][1]); }
                        // note : inner z (nZ A)
                        else if (good_comb_rz_vec[track_i][1] < 0 )    { Good_inner_outer_pos_xy_nzA -> Fill(good_comb_xy_vec[track_i][0],good_comb_xy_vec[track_i][1]); }
                        // note : inner z (pZ A)
                        else if (good_comb_rz_vec[track_i][1] < 130 )  { Good_inner_outer_pos_xy_pzA -> Fill(good_comb_xy_vec[track_i][0],good_comb_xy_vec[track_i][1]); }
                        // note : inner z (pZ B)
                        else                                           { Good_inner_outer_pos_xy_pzB -> Fill(good_comb_xy_vec[track_i][0],good_comb_xy_vec[track_i][1]); }
                        
                        
                        // note : Outer z (nZ B)
                        if      (good_comb_rz_vec[track_i][3] < -130 ) { Good_inner_outer_pos_xy_nzB -> Fill(good_comb_xy_vec[track_i][2],good_comb_xy_vec[track_i][3]); }
                        // note : Outer z (nZ A)
                        else if (good_comb_rz_vec[track_i][3] < 0 )    { Good_inner_outer_pos_xy_nzA -> Fill(good_comb_xy_vec[track_i][2],good_comb_xy_vec[track_i][3]); }
                        // note : Outer z (pZ A)
                        else if (good_comb_rz_vec[track_i][3] < 130 )  { Good_inner_outer_pos_xy_pzA -> Fill(good_comb_xy_vec[track_i][2],good_comb_xy_vec[track_i][3]); }
                        // note : Outer z (pZ B)
                        else                                           { Good_inner_outer_pos_xy_pzB -> Fill(good_comb_xy_vec[track_i][2],good_comb_xy_vec[track_i][3]); }


                        // todo : add the cluster position cut probably
                        dNdeta_hist -> Fill(track_eta);
                        if (good_comb_phi_vec[track_i][0] > 104 && good_comb_phi_vec[track_i][0] < 211) {
                            dNdeta_2P_inner_hist -> Fill(track_eta_inner);
                            
                            track_line -> DrawLine(temp_event_zvtx_info[0], sqrt(pow(beam_origin.first,2)+pow(beam_origin.second,2)), good_comb_rz_vec[track_i][1], good_comb_rz_vec[track_i][0]);
                        }

                        if (good_comb_phi_vec[track_i][1] > 104 && good_comb_phi_vec[track_i][1] < 211) dNdeta_2P_outer_hist -> Fill(track_eta_outer);

                    }

                } // note : good track for loop

                c4 -> Print( Form("%s/folder_%s/good_track/good_track_%i.pdf",mother_folder_directory.c_str(),file_name.c_str(),event_i) );
                c4 -> Clear();
        ...
        if ( zvtx_draw_requireL < temp_event_zvtx_vec.size() && draw_event_display == true && temp_event_zvtx_vec.size() > zvtx_cal_require)
        {   
            ...
            if(draw_event_display && (event_i % print_rate) == 0){c2 -> Print(Form("%s/folder_%s/temp_event_display.pdf",mother_folder_directory.c_str(),file_name.c_str()));}
            pad_xy -> Clear();
            pad_rz -> Clear();
            pad_z  -> Clear();
    ...
    c1 -> Print(Form("%s/folder_%s/zvtx_evt_width.pdf",mother_folder_directory.c_str(),file_name.c_str()));
    c1 -> Clear();
    ...
    c1 -> Print(Form("%s/folder_%s/Good_inner_outer_pos_xy_nzB.pdf",mother_folder_directory.c_str(),file_name.c_str()));
    c1 -> Clear();
    Good_inner_outer_pos_xy_nzA-> Draw("colz0");
    c1 -> Print(Form("%s/folder_%s/Good_inner_outer_pos_xy_nzA.pdf",mother_folder_directory.c_str(),file_name.c_str()));
    c1 -> Clear();
    Good_inner_outer_pos_xy_pzA-> Draw("colz0");
    c1 -> Print(Form("%s/folder_%s/Good_inner_outer_pos_xy_pzA.pdf",mother_folder_directory.c_str(),file_name.c_str()));
    c1 -> Clear();
    Good_inner_outer_pos_xy_pzB-> Draw("colz0");
    c1 -> Print(Form("%s/folder_%s/Good_inner_outer_pos_xy_pzB.pdf",mother_folder_directory.c_str(),file_name.c_str()));
    c1 -> Clear();
    ...
    c1 -> Print(Form("%s/folder_%s/good_tracklet_rz.pdf",mother_folder_directory.c_str(),file_name.c_str()));
    c1 -> Clear();
    ...
    c3 -> Print(Form("%s/folder_%s/dNdeta.pdf",mother_folder_directory.c_str(),file_name.c_str()));
    pad_obj -> Clear();
    ...
    c3 -> Print(Form("%s/folder_%s/dNdeta_2P_inner.pdf",mother_folder_directory.c_str(),file_name.c_str()));
    pad_obj -> Clear();
    ...
    c3 -> Print(Form("%s/folder_%s/dNdeta_2P_outer.pdf",mother_folder_directory.c_str(),file_name.c_str()));
    pad_obj -> Clear();
