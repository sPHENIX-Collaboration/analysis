...
void INTT_xyvtx(string run_ID, int geo_mode_id, string file_event, bool full_event, int run_Nevent)
{
    ...
    vector<double>* server_vec = new vector<double>();
    vector<double>* module_vec = new vector<double>();
    ...
    for (int event_i = 0; event_i < N_event; event_i++)
    {
        ...
        for (int clu_i = 0; clu_i < length; clu_i++)
        {
            ...
            // note : for the test
            if (server_vec -> at(clu_i) == 3002 && module_vec -> at(clu_i) == 3){
                temp_sPH_outer_nocolumn_vec.push_back({
                    ...
                    z_vec -> at(clu_i), 
                    layer_vec -> at(clu_i), 
                    ((y_vec -> at(clu_i) + 0.25) < 0) ? atan2((y_vec -> at(clu_i) + 0.25), (x_vec -> at(clu_i) + 0.25) ) * (180./TMath::Pi()) + 360 : atan2((y_vec -> at(clu_i) + 0.25), (x_vec -> at(clu_i) + 0.25) ) * (180./TMath::Pi())
                });    
            }

            else if (layer_vec -> at(clu_i) == 0){ // note : inner
            ...
            else if (layer_vec -> at(clu_i) == 1) { // note : outer
                temp_sPH_outer_nocolumn_vec.push_back({
                    column_vec -> at(clu_i), 
                    avg_chan_vec -> at(clu_i), 
                    sum_adc_vec -> at(clu_i), 
        ...
        for ( int inner_i = 0; inner_i < temp_sPH_inner_nocolumn_vec.size(); inner_i++ )
        {
            if (temp_sPH_inner_nocolumn_vec[inner_i].z > 0) continue;
            
            for ( int outer_i = 0; outer_i < temp_sPH_outer_nocolumn_vec.size(); outer_i++ )
            {
                if (temp_sPH_outer_nocolumn_vec[outer_i].z > 0) continue;
                // bool DCA_tag = false;
