...
void test_z_382()
{
    ...
    string out_folder_directory = input_directory + "/codetest_SETest_" + file_name;
    ...
    for (int event_i = 24; event_i < 25; event_i ++)
    {
        INTTClu -> EvtInit(event_i);
        INTTClu -> EvtSetCluGroup();

        cout<<" true z vertex : "<<INTTClu -> GetTrigZvtxMC()<<endl;
        cout<<"inner Nclus : "<<INTTClu -> temp_sPH_inner_nocolumn_vec.size()<<" outer Nclus : "<<INTTClu -> temp_sPH_outer_nocolumn_vec.size()<<endl;
        ...
        // cout<<"event NClus : "<<INTTClu -> temp_sPH_inner_nocolumn_vec.size() + INTTClu -> temp_sPH_outer_nocolumn_vec.size()<<" true zvtx : "<<INTTClu -> GetTrigZvtxMC()<<endl;
        // cout<<"event reco zvtx : "<<MCz -> GetEvtZPeak()[0]<<" "<<MCz -> GetEvtZPeak()[1]<<" "<<MCz -> GetEvtZPeak()[2]<<" "<<endl;
