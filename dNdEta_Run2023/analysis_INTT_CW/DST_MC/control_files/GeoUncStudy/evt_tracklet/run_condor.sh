...
submit_condor_job () {
    ...
    cp run_root_mother.sh run_root_sub_evtTracklet_${i}.sh
    sed -i "s/the_file_number/${i}/g" run_root_sub_evtTracklet_${i}.sh
    chmod +777 run_root_sub_evtTracklet_${i}.sh
    ...
    # rm run_root_sub_evtTracklet_${i}.sh
    sleep 1
}
...
do
    current_condor_job=$(( `condor_q | grep run_root_sub_evtTracklet | wc -l` - 0 ))
    ...
    if [ $current_condor_job -lt 500 ]; then
        ...
        do
            ...
            waiting_loop=0
            while [ ! -f /sphenix/user/ChengWei/sPH_dNdeta/Sim_Ntuple_HIJING_new_20240424_HR_test/Geo_test/evt_vtxZ/complete_file/INTT_zvtx_${file_id}.root ]; do
                echo the file INTT_zvtx_${file_id}.root does not exist waiting!
