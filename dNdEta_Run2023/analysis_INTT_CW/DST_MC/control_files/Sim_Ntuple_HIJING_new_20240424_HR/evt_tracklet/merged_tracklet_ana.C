...
int merged_tracklet_ana()
{
    ...
    string input_directory2 = "/sphenix/user/ChengWei/sPH_dNdeta/Sim_Ntuple_HIJING_new_20240424_HR/evt_tracklet/complete_file/merged_file_folder_cm_radian_test";
    ...
    EtaDistReader * file_half_corr = new EtaDistReader(run_type, out_folder_directory1, fulleta_MC_included_z_bin, included_eta_z_map, Form("%s/%s",input_directory1.c_str(), input_file1.c_str()), Mbin_Z_evt_map_name_bool1);
    ...
    EtaDistReader * file_half_test = new EtaDistReader(run_type, out_folder_directory2, fulleta_MC_included_z_bin, included_eta_z_map, Form("%s/%s",input_directory2.c_str(), input_file2.c_str()), Mbin_Z_evt_map_name_bool2);
    ...
    {
        dNdeta_1D_fulleta_MC_1[i] -> SetLineColor(TColor::GetColor("#1A3947"));
        ...
        dNdeta_1D_fulleta_MC_1[i] -> SetMaximum( dNdeta_1D_fulleta_MC_1[i] -> GetBinContent(dNdeta_1D_fulleta_MC_1[i] -> GetMaximumBin()) * 1.3 );

        dNdeta_1D_fulleta_MC_1[i] -> Draw("hist");
        dNdeta_1D_fulleta_MC_2[i] -> Draw("hist same");

        dNdeta_1D_reco_single_original_1[i] -> SetMarkerColor(TColor::GetColor("#1A3947"));
        dNdeta_1D_reco_single_original_1[i] -> SetLineColor(TColor::GetColor("#1A3947"));
    ...
    for (int i = 0; i < dNdeta_1D_fulleta_MC_1.size(); i++)
    {
        ...
        dNdeta_1D_fulleta_MC_1[i] -> SetMinimum(0);
        dNdeta_1D_fulleta_MC_1[i] -> SetMaximum( dNdeta_1D_fulleta_MC_1[i] -> GetMaximum() * 1.4 );

        dNdeta_1D_fulleta_MC_1[i] -> Draw("hist");
        dNdeta_1D_fulleta_MC_2[i] -> Draw("hist same");

        dNdeta_1D_reco_multi_original_1[i] -> SetMarkerColor(TColor::GetColor("#1A3947"));
        dNdeta_1D_reco_multi_original_1[i] -> SetLineColor(TColor::GetColor("#1A3947"));
