...
void test_xy_398_000_geo(int loop_i, int core_i)
{
    ...
    int N_loop = 20;   // note : number of times, the core runs the job
    int N_trial = 500; // note : in each job, the number of loops that the residual of geometry is given
    ...
    tree_out -> Branch("angle_diff_mean",     &angle_diff_mean);
    ...
    angle_diff_new_bkg_remove_final = new TH1F("","angle_diff_new_bkg_remove_final",100, 0, 3);
    ...
    for (int trial_i = 0; trial_i < N_trial; trial_i++)
    {
        ...
        for (int layer_i = 3; layer_i < 7; layer_i++)
        {
            ...
            for (int phi_i = 0; phi_i < N_layer_ladder; phi_i++)
            {
                ...
                total_offset_x += fabs(ladder_offset_map[Form("%i_%i", layer_i, phi_i)].first);
                total_offset_y += fabs(ladder_offset_map[Form("%i_%i", layer_i, phi_i)].second);
            }
        }
        ...
        DCA_distance_mean   = out_vtx[7].first;
        DCA_distance_stddev = out_vtx[7].second;

        angle_diff_mean   = out_vtx[8].first;  // note : with bkg removal
        angle_diff_stddev = out_vtx[8].second; // note : with bkg removal

        // cout<<"test2"<<endl;
        MCxy -> TH2F_FakeClone(MCxy -> GetHistFinal().first[0], DCA_distance_inner_phi_peak_final);
        MCxy -> TH2F_FakeClone(MCxy -> GetHistFinal().first[1], angle_diff_inner_phi_peak_final  );
        MCxy -> TH2F_FakeClone(MCxy -> GetHistFinal().first[2], DCA_distance_outer_phi_peak_final);
