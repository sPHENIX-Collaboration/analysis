...
void evtTracklet_mother(int core_i, string output_sub_folder_name = "evt_tracklet", string used_zvtx_folder_name = "evt_vtxZ")
{
    ...
    for (int event_i = 0; event_i < INTTClu -> GetNEvt(); event_i ++)
    {
        ...
        // if (RecoZ -> LB_Gaus_Mean_mean < -205 || RecoZ -> LB_Gaus_Mean_mean > -195) {MCEta -> ClearEvt(); INTTClu -> EvtClear(); continue;} // note : special_tag cancel
        ...
        MCEta     -> ProcessEvt(
            ...
            1 // note : special_tag, no zvtx reweight
        );
