...
void avgXY_mother(int core_i)
{
    ...
    pair<double, double> beam_origin = {0.,-2.}; // note : note used 
    ...
    int clu_size_cut = 8;     // note : used 
    int N_clu_cut = 350;      // note : used 
    int N_clu_cutl = 20;      // note : used 
    
    bool draw_event_display = false;
    double peek = 3.324;
    int data_type = 6; // note : MC selfgen
    int geo_mode_id = 1; // note : 0 -> perfect geo not used
    ...
    int start_evt = core_i * 10000;
    int end_evt   = (core_i + 1) * 10000;
    
    if (start_evt >= INTTClu -> GetNEvt()) { return; }
    if (fabs(start_evt - INTTClu -> GetNEvt()) < 10000) { return; } // note : because the previous will be handling the remaining events
    if (fabs(end_evt - INTTClu -> GetNEvt()) < 10000) { end_evt = INTTClu -> GetNEvt();}
