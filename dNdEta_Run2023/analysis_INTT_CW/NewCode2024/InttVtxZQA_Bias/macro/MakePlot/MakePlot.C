...
)
{
    ...
    sub_ltx->DrawLatex(0.45, 0.78, Form("Mean diff (PostQA - NoQA)/NoQA: %.3f", (data_hist_vec_in[0].second->GetMean() - MC_hist_vec_in[0].second->GetMean())/MC_hist_vec_in[0].second->GetMean()));
    sub_ltx->DrawLatex(0.45, 0.74, Form("StdDev diff (PostQA - NoQA)/NoQA: %.3f", (data_hist_vec_in[0].second->GetStdDev() - MC_hist_vec_in[0].second->GetStdDev())/MC_hist_vec_in[0].second->GetStdDev()));
...
int MakePlot()
{
    ...
    for (int i = 0; i < 15; i++)
    {
        comp_name.push_back(Form("h1D_NClus_Mbin%d",i));
    }

    TFile * file_in = TFile::Open(Form("%s/%s", input_directory.c_str(), input_file_name.c_str()), "READ");
    if (!file_in)
    {
        std::cout<<"Error: file_in is nullptr"<<std::endl;
        exit(1);
    }

    for (std::string hist_name : comp_name)
    {
