void ParticleShowerSizeHistoMaker(const int nevents)
{
 ...
 TH2F *h2 = new TH2F("h2","Particle Shower Sizes",nchan,min,max,nchan,min,max);
  h2->SetXTitle("dtheta");
  h2->SetYTitle("dphi");
...
de1->Project("h2","dphi:dtheta","(ID<=2)*edep","",nevents);
  ...
  {
de1->Project("h2","dphi:dtheta","(ID<=2)*edep","");
  }
  h2->Draw("surf");
  f1->Close();
  ...
  cout << nevents << endl;
  TH2F *h2a = new TH2F("h2a","Particle Shower Sizes",nchan,min,max,nchan,min,max);
  ...
  {
  de2->Project("h2a","dphi:dtheta","(ID<=2)*edep","");
  }
