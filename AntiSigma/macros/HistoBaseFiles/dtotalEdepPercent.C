void EdepPercentSmall (const char *calo="HO", const char *part="e")
{
  ...
  // double x[9];
 char hname [100];
 sprintf(hname, "%s%sd",calo,part/*,pz[i]*/);
 TH1F *h1 = new TH1F(hname, hname, 100, 0, 10);
  for (int i=0; i<3; i++)
{
  //x[i] = pz[i];
  sprintf(infile, "/sphenix/user/jpinkenburg/ShowerSize/ntuple/%s_eta0_4GeV.root",part/*, pz[i]*/);
	      char hname [100];
	      // sprintf(hname, "%s%s12GeV",calo,part,pz[i]);
	      //TH1F *h1 = new TH1F(hname, "", 100, 0, 1);
	      TFile *f = TFile::Open(infile);
	      gROOT->cd();
	      TNtuple *nt = (TNtuple *)f->Get("de");
	      char calid [100];
	      sprintf(calid, "ID==%s", i); 
	      nt->Project(hname,"edep",calid);
	      // char cal [100];
	      // sprintf(cal,"%s",calo);
	      /*
	      if (calo == "HO")
		{
		  nt->Project(hname,"edep","ID==2");
		}
	      else if(calo == "HI")
		{ 
		  nt->Project(hname,"edep","ID==1");
		} 
	      else if(calo == "E")
		{ 
		  nt->Project(hname,"edep","ID==0");
		}
	      else if (calo == "MAG") 
		{
	  ...
	  nt->Project(hname,"(BH)/(EA+ES+HIA+HIS+HOA+HOS+BH+MAG)");
	      ...
	      else 
		{
	      ...
	      h1->Draw();
	      char fname [100];
	      sprintf(fname, "%sShowerPercent.root",part);
