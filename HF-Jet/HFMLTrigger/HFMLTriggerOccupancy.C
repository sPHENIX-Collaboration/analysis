...
//      m_hitPixelZMap->Fill(pixelZIndex, world_coords.z(), halfLayerIndex);
      ...
      assert(static_cast<size_t>(cell->get_stave_index()) < multiplicity_layer.size());
      auto& multiplicity_stave = multiplicity_layer[cell->get_stave_index()];
      assert(static_cast<size_t>(cell->get_chip_index()) < multiplicity_stave.size());
      multiplicity_stave[cell->get_chip_index()]++;
