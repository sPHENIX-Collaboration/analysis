...
int caloHistGen::process_event(PHCompositeNode *topNode)
{
  ...
  if (!vertexmap)
  {
    ...
    if (vertexmap->empty())
    {
        std::cout << "caloHistGen::process_event - Fatal Error - GlobalVertexMap node is empty. Please turn on the do_global flag in the main macro  in order to reconstruct the global vertex." << std::endl;
