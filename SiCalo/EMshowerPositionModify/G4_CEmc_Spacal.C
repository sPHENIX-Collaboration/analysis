...
void CEMC_Clusters()
{
  ...
  {
    // RawClusterBuilderTemplate *ClusterBuilder1 = new RawClusterBuilderTemplate("EmcRawClusterBuilderTemplate1");
    // ClusterBuilder1->Detector("CEMC");
    // ClusterBuilder1->setUseRawTowerGeomv5(false);
    // ClusterBuilder1->setProjectToInnerSurface(false);
    // ClusterBuilder1->Verbosity(verbosity);
    // ClusterBuilder1->set_threshold_energy(0.030);  // This threshold should be the same as in CEMCprof_Thresh**.root file below
    // std::string emc_prof = getenv("CALIBRATIONROOT");
    // emc_prof += "/EmcProfile/CEMCprof_Thresh30MeV.root";
    // ClusterBuilder1->LoadProfile(emc_prof);
    // if (!Enable::CEMC_G4Hit) ClusterBuilder1->set_UseTowerInfo(1);  // just use towerinfo
    // //    ClusterBuilder1->set_UseTowerInfo(1); // to use towerinfo objects rather than old RawTower
    ...
    // ClusterBuilder2->setUseRawTowerGeomv5(true);
    // ClusterBuilder2->setProjectToInnerSurface(true);
    ...
    // std::string emc_prof = getenv("CALIBRATIONROOT");
    // emc_prof += "/EmcProfile/CEMCprof_Thresh30MeV.root";
    // ClusterBuilder2->LoadProfile(emc_prof);
    // // if (!Enable::CEMC_G4Hit) ClusterBuilder2->set_UseTowerInfo(1);  // just use towerinfo
    // //    ClusterBuilder2->set_UseTowerInfo(1); // to use towerinfo objects rather than old RawTower
    // se->registerSubsystem(ClusterBuilder2);

    RawClusterBuilderTemplate *ClusterBuilder_detailed = new RawClusterBuilderTemplate("EmcRawClusterBuilderTemplate_detailed");
    ClusterBuilder_detailed->Detector("CEMC");
    // ClusterBuilder_detailed->set_UseDetailedGeometry(true);
    ClusterBuilder_detailed->set_threshold_energy(0.070);  // for when using basic calibration
    std::string emc_prof = getenv("CALIBRATIONROOT");
    emc_prof += "/EmcProfile/CEMCprof_Thresh30MeV.root";
    // ClusterBuilder->set_UseAltZVertex(3); //0: GlobalVertexMap, 1: MbdVertexMap, 2: Nothing, 3: G4TruthInfo
    ClusterBuilder_detailed->LoadProfile(emc_prof);
    // ClusterBuilder_detailed->set_UseTowerInfo(1);  // to use towerinfo objects rather than old RawTower
    se->registerSubsystem(ClusterBuilder_detailed);

    // RawClusterBuilderTemplate *ClusterBuilder_proj = new RawClusterBuilderTemplate("EmcRawClusterBuilderTemplate_proj");
    // ClusterBuilder_proj->Detector("CEMC");
    // ClusterBuilder_proj->set_UseDetailedGeometry(false);
    // ClusterBuilder_proj->set_threshold_energy(0.070);  // for when using basic calibration
    // std::string emc_prof = getenv("CALIBRATIONROOT");
    // emc_prof += "/EmcProfile/CEMCprof_Thresh30MeV.root";
    // // ClusterBuilder->set_UseAltZVertex(3); //0: GlobalVertexMap, 1: MbdVertexMap, 2: Nothing, 3: G4TruthInfo
    // ClusterBuilder_proj->LoadProfile(emc_prof);
    // // ClusterBuilder_proj->set_UseTowerInfo(1);  // to use towerinfo objects rather than old RawTower
    // se->registerSubsystem(ClusterBuilder_proj);
  }
