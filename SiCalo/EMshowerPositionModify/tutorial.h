...
private:
    ...
    int prepareTrackStates(PHCompositeNode* topNode);
    ...
    SvtxTrackState *emcProjState = nullptr;
    ...
    // note : proj trkr to calo 
    std::vector <double> track_proj_X;
    std::vector <double> track_proj_Y;
    std::vector <double> track_proj_Z;
    std::vector <double> track_proj_R;
    std::vector <double> track_proj_Phi;
    ...
    // energy-weighted CoG from CEMC G4Hits in this event
    std::vector<float> _CEMC_Hit_CoG_x;
    std::vector<float> _CEMC_Hit_CoG_y;
    std::vector<float> _CEMC_Hit_CoG_z;
    std::vector<float> _CEMC_Hit_CoG_R;
    std::vector<float> _CEMC_Hit_CoG_Edep;
    ...
    std::set<int> primary_particles_tracks;

    struct FirstHit {
        double t0;              // the trackid g4hit的最早时间
        double x0, y0, z0, r0; // 入口点信息
        bool   set = false;
    };
    std::unordered_map<int, FirstHit> first_by_tid;
