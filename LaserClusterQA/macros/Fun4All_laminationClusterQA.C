...
#include <laserclusterqa/laminationClusterQA.h>
...
R__LOAD_LIBRARY(libLaserClusterQA.so)
void Fun4All_laminationClusterQA(
...

{
  ...
  laminationClusterQA *lcQA = new laminationClusterQA("laminationClusterQA");
  lcQA->setOutfile(Form("%s/laminationHitQA_%s_%s-%08d.root",outdir.c_str(),type.c_str(),prod.c_str(),runnumber));
  se->registerSubsystem(lcQA);
