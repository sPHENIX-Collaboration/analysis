...
RICHParticleID::process_event(PHCompositeNode *topNode)
{
  ...
  for (SvtxTrackMap::ConstIter track_itr = trackmap->begin(); track_itr != trackmap->end(); track_itr++)
	...
	continue;
      ...
      // Estimate mean emission point instead of real mean track point (temporary) */
...
bool
RICHParticleID::get_true_momentum( PHG4TruthInfoContainer* truthinfo, SvtxTrack_FastSim * track, double arr_mom[3] )
{
  arr_mom[0] = 0;
  arr_mom[1] = 0;
  arr_mom[2] = 0;
