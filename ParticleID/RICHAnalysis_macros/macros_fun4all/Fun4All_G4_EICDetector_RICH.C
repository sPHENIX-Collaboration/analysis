                                ...
                                )
{
  ...
  bool do_EEMC_cluster = do_EEMC_twr && false;
  ...
  const float magfield_rescale = 0; // scale the map to a 0 T field
  ...
  if (readhits)
  ...
  else if (readeictree)
    {
      ...
      eicr->SetFirstEntry(0); // first entry to be processed in input file; start event counting at 0
  ...
  if (usegun)
    {
      ...
      pgen->set_name("proton");
      pgen->set_z_range(0,0);
      pgen->set_eta_range(2,2);
      pgen->set_mom_range(set_momentum,set_momentum);
      //      pgen->set_mom_range(20,20);
