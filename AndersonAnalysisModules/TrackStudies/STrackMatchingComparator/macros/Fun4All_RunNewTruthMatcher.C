...
int Fun4All_RunNewTruthMatcher(
    const int nEvents = 1,
    const int skip = 0,
    ...
    const string &outdir = ".")
{
  ...
  if (Input::SIMPLE)
  {
    INPUTGENERATOR::SimpleEventGenerator[0]->add_particles(sParticle.Data(), nPerEvt);
    ...
    INPUTGENERATOR::SimpleEventGenerator[0]->set_pt_range(ptParMin, ptParMax);
  }
  // Upsilons
...
//  G4MAGNET::magfield_rescale = 1.;  // make consistent with expected Babar field strength of 1.4T
  ...
  if (true) {
    ...
    std::cout << "CHECK I am inside the svtx evaluator block!" << std::endl;

    SvtxEvaluator* eval = new SvtxEvaluator(
      "SVTXEVALUATOR",
      outputroot + "_g4svtx_eval.root",
      "SvtxTrackMap",
      G4MVTX::n_maps_layer,
      G4INTT::n_intt_layer,
      G4TPC::n_gas_layer,
    ...
    eval->Verbosity(Enable::TRACKING_VERBOSITY);
    se->registerSubsystem(eval);
  }  // end if (Enable::TRACKING_EVAL)
  ...
  if (Enable::TRACK_MATCHING) {
    ...
    trackmatcher->set_max_nreco_per_truth (1); // maximum reco tracks matched for any truth track // TEST [Derek, 10.13.2023]
    trackmatcher->set_max_ntruth_per_reco (1); // maximum truth tracks matched for any reco track // TEST [Derek, 10.13.2023]
    ...
    if (Enable::TRACK_MATCHING_TREE) {
      auto treefiller = new FillClusMatchTree(ismatcher, outputroot + "_g4trackmatching.root");//, true, true, true, outputFile);
