...
if [ "$iter" -eq 0 ]; then
  root -b "/sphenix/u/bseidlitz/work/cvpDev/analysis/EMCal_pi0_Calib_2023/macros/Fun4All_EMCal.C(0,\"inputdata.txt\",0,\"local_calib_copy.root\")"
  iter=$((iter+1))
fi
...
if [ "$iter" -eq 1 ]; then
  root -b "../doTscFit.C(\"${hist_out}\",\"local_calib_copy.root\")"
else
