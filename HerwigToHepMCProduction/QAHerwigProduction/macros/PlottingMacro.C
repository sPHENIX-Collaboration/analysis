...
void DoAllThePlotting(TFile* herwig_file, TFile* pythia_file, float herwig_xs=1., float pythia_xs=1.)
{
...
void PlotJetPlots(TDirectory* herwig_jets, TDirectory* pythia_jets, std::vector<TCanvas*>*Canvi)
{
	...
	CollectJets(herwig_jets, herwig_jet_array, herwig_lead_jet_array, herwig_n_jets);
	//all Jets
	...
	CollectJets(pythia_jets, pythia_jet_array, pythia_lead_jet_array, pythia_n_jets);
...
void CollectJets(TDirectory* jets, std::array<std::vector<TH1F*>*, 4>* all_jets, std::array<std::vector<TH1F*>*, 4>* lead_jets, std::vector<TH1F*>* all_jet_n)
{
	for(int i=2; i<7; i++)
	{
		jets->cd();
		...
		rdir->cd();
		all_jets->at(0)->push_back((TH1F*)r_dir->Get(std::format("h_jets_r0{}_pt", i).c_str());
		all_jets->at(1)->push_back((TH1F*)r_dir->Get(std::format("h_jets_r0{}_e", i).c_str());
		all_jets->at(2)->push_back((TH1F*)r_dir->Get(std::format("h_jets_r0{}_phi", i).c_str());
		all_jets->at(3)->push_back((TH1F*)r_dir->Get(std::format("h_jets_r0{}_eta", i).c_str());
		all_jet_n->push_back((TH1I*)r_dir->Get(std::format("h_jets_r0{}_n",i).c_str())i);
		TDirectory* leaddir=(TDirectory*)r_dir->GetDirectory("Lead_Jets");
		lead_jets->at(0)->push_back((TH1F*)r_dir->Get(std::format("h_lead_jets_r0{}_pt", i).c_str());
		lead_jets->at(1)->push_back((TH1F*)r_dir->Get(std::format("h_lead_jets_r0{}_e", i).c_str());
		lead_jets->at(2)->push_back((TH1F*)r_dir->Get(std::format("h_lead_jets_r0{}_phi", i).c_str());
		lead_jets->at(3)->push_back((TH1F*)r_dir->Get(std::format("h_lead_jets_r0{}_eta", i).c_str());
		rdir->cd();
	}	
	return;
...
void PlotJetObs(std::vector<TH1F*>* herwig_obs, std::vector<TH1F*>* pythia_obs, TCanvas* obs_canv)
{
	...
	TLegend* l_data=new TLegend(0.8, 0.3, 1, 0.5);
	...
	{
		pads->at(0)->cd();
		herwig_obs->at(i)->SetLineColor(i);
		herwig_obs->at(i)->SetMarkerColor(i);
		herwig_obs->at(i)->SetMarkerStyle(i);
		herwig_obs->at(i)->Draw("same");
		pythia_obs->at(i)->SetLineColor(i);
		pythia_obs->at(i)->SetMarkerColor(i);
		pythia_obs->at(i)->SetMarkerStyle(i+1);
	}
	return;
}
