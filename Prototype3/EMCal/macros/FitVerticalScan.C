...
vector<TGraph *> vg_src;

void OnlineAnalysis(TString filename = "/phenix/u/jinhuang/links/sPHENIX_work/Prototype_2016/column3_214x.txt")
{
  ...
  TLegend *leg = new TLegend(0.9122257,0.6483516,0.9968652,0.9010989,NULL,"brNDC");
  leg->SetFillColor(0);
  ...
  while ( sumfile >> rn >> tower >> mpv >> dmpv  ) {
    index = (tower/8);
    cout << index << " " << rn << " " << runtoz[rn] << " " << mpv << " " << dmpv << endl;
  ...
  int marker = 21;
  for ( int i = 0; i < 8; i++ ) {
    gr[i] = new TGraphErrors( v_x[i].size(), &v_x[i][0], &v_y[i][0], &v_dx[i][0], &v_dy[i][0] );
    ...
    vg_src.push_back( ( TGraph *)gr[i]->Clone() );
...
//  int i = 1;
//  vg_src.push_back((TGraph *) c2->GetListOfPrimitives()->At(i++)->Clone());
//  vg_src.push_back((TGraph *) c2->GetListOfPrimitives()->At(i++)->Clone());
//  vg_src.push_back((TGraph *) c2->GetListOfPrimitives()->At(i++)->Clone());
//  vg_src.push_back((TGraph *) c2->GetListOfPrimitives()->At(i++)->Clone());
//  vg_src.push_back((TGraph *) c2->GetListOfPrimitives()->At(i++)->Clone());
...
//  vg_src.push_back((TGraph *) c2->GetListOfPrimitives()->At(i++)->Clone());
  ...
  TGraph * g_final = new TGraph((vg_src[0])->GetN());
...
//  f_att->SetLineWidth(4);
  TF1 * f_att = new TF1("f_att","(exp(-((14-x)/[0])) + 0.3 * exp(-((x-1 + 13)/[0])) )*[1]",0,20);
  f_att->SetParameters(105,1);
  f_att->SetLineColor(kBlue+3);
  f_att->SetLineWidth(4);

  TGraph * g_src = vg_src[0];
//  g_src->Print();
  for (int j = 0; j < (g_src->GetN()); ++j)
    {
      ...
      (g_final->GetY())[j] = 0;
      (g_final->GetX())[j] = (g_src->GetX())[j]/10;
    }

  for (int i = 0; i < 8; ++i)
    {
      TGraph * g_src = vg_src[i];
  ...
  TH1 * frame = gPad->DrawFrame(0, .9, 15, 1.1);
  frame->SetTitle(";Beam position along the length of module (cm);MIP signal amplitude (A. U.)");
