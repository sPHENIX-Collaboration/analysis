...
//        "/phenix/u/jinhuang/links/sPHENIX_work/Prototype_2017/ShowerCalib/2nd_tower45.lst_EMCalCalib.root"//
      "testbeam_simdata_withposrecal.root"
...
//    SimPositionCheck(-40+3); // 45 degree tilted
  Get_Res_linear_Summmary_Sim("recalib");
}
...
void
Get_Res_linear_Summmary_Sim(TString clustype = "recalib")
{
...
//  return;
  ...
  lin_res ges_clus_5x5_prod = GetResolution("clus_5x5_"+clustype, beam_mom, kBlue,"sum_E");
 
  lin_res ges_clus_3x3_prod = GetResolution("clus_3x3_"+clustype, beam_mom,
      kBlue + 3,"sum_E");
  cout<<ges_clus_3x3_prod.name<<endl;
  TCanvas *c1 = new TCanvas(Form("Res_linear") + cuts,
      Form("Res_linear") + cuts, 1300, 600);
  c1->Divide(2, 1);
  int idx = 1;
...
//  f_calo_l->Draw("same");
  ...
  name5.str("");
  name3.str("");
  name5<<"clus_5x5_"<<clustype;
  name3<<"clus_3x3_"<<clustype;
  leg->AddEntry(ges_clus_5x5_prod.linearity, name5.str().c_str(), "ep");
  leg->AddEntry(ges_clus_3x3_prod.linearity, name3.str().c_str(), "ep");
  leg->AddEntry(f_calo_l_sim, "Unity", "l");
  leg->Draw();
...
//  p->SetLogy();
  ...
  TF1 * f_calo_sim = new TF1("f_calo_sim", "sqrt([0]*[0]+[1]*[1]/x)/100", 0.5,
      25);
  //updated by Joe from sPHENIX perfect sim conditions
  f_calo_sim->SetParameters(2.1, 11.8);
  f_calo_sim->SetLineWidth(3);
  f_calo_sim->SetLineColor(kGreen + 2);
  ...
  leg->AddEntry(ges_clus_5x5_prod.resolution, name5.str().c_str(), "ep");
  leg->AddEntry(ges_clus_5x5_prod.f_res,
      Form("#DeltaE/E = %.1f%% #oplus %.1f%%/#sqrt{E}",
          ges_clus_5x5_prod.f_res->GetParameter(0),
          ges_clus_5x5_prod.f_res->GetParameter(1)), "l");

  leg->AddEntry(ges_clus_3x3_prod.resolution, name3.str().c_str(), "ep");
  leg->AddEntry(ges_clus_3x3_prod.f_res,
      Form("#DeltaE/E = %.1f%% #oplus %.1f%%/#sqrt{E}",
          ges_clus_3x3_prod.f_res->GetParameter(0),
          ges_clus_3x3_prod.f_res->GetParameter(1)), "l");
...
//  leg->AddEntry((TObject*) 0, " ", "");
  ...
  leg->AddEntry(f_calo_sim,
      Form("Perfect Sim., #DeltaE/E = %.1f%% #oplus %.1f%%/#sqrt{E}",
          f_calo_sim->GetParameter(0), f_calo_sim->GetParameter(1)), "l");
  leg->Draw();
