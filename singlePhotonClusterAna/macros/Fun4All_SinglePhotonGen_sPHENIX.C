...
int Fun4All_SinglePhotonGen_sPHENIX(
    const int nEvents = 1,
    const int pid = 22,
    ...
    const string &outdir = ".")
{
  ...
  if (Input::SIMPLE)
  {
    INPUTGENERATOR::SimpleEventGenerator[0]->add_particles(pid, 1);
    ...
    INPUTGENERATOR::SimpleEventGenerator[0]->set_p_range(2.0, 20.0);
  }
  // Upsilons
...
//  G4MAGNET::magfield_rescale = 1.;  // make consistent with expected Babar field strength of 1.4T
  ...
  singlePhotonClusterAna *eval = new singlePhotonClusterAna("dummy", outputroot + "_singlePhotonClusterAna.root");
