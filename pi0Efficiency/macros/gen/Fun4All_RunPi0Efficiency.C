...
#include <../src/pi0Efficiency.h>

R__LOAD_LIBRARY(libfun4all.so)
R__LOAD_LIBRARY(libpi0Efficiency.so)

void Fun4All_RunPi0Efficiency(
      const int nEvents = 1,
      const string &clusterFile = "dst_calo_cluster.list",
      const string &truthFile = "dst_truth.list",
      const string &outputFile = "cemcPi0Efficiency_Out.root"
			)
{
  ...
  pi0Efficiency *eval = new pi0Efficiency("dummy", outputFile);
