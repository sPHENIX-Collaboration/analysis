...
#     <set>          list folder under lists/, made by lists/make_lists.bash (e.g. run_ecuts_cf)
#     [array-range]  optional, e.g. 0-3 or 7 (default: all lists, 0..nlists-1)
# Snapshots the current corral_m to log/<set>/corral_m.snapshot so every task runs the same binary.
# Outputs: root/<set>/corral_m_NN.root, pdf/<set>/corral_m_NN.pdf, log/<set>/corral_m_NN.log
...
RANGE=${2:-0-$((NL-1))}
mkdir -p $PROJDIR/root/$SET $PROJDIR/pdf/$SET $PROJDIR/log/$SET
cp -v $PROJDIR/corral_m $PROJDIR/log/$SET/corral_m.snapshot
echo "set $SET: $NL lists, submitting array $RANGE"
sbatch --partition=prip --time=2-23:59:59 \
	--array=$RANGE \
	--job-name=crm_$SET \
	--output=$PROJDIR/log/$SET/corral_m_%2a.log \
	--error=$PROJDIR/log/$SET/corral_m_%2a.log \
	--mem=8G \
	--export=ALL \
