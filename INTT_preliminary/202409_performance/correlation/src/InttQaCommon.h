...
namespace InttQa
{
  ...
  void HistsConfig( int hist_num, TH* hists )
  {
	...
	for( int j=1; j<hists[i]->GetNbinsX()+1; j++ )
	  {
		...
		//cout << hists[i]->GetBinContent( j ) << endl;
	      }
	  }
      }
    ...
    // if no entry is found in the hostograms, set a dummy value
    if( max_non_zero_x < hists[0]->GetNbinsX() )
      max_non_zero_x++;
    // else if( max_non_zero_x >= hists[0]->GetNbinsX() ) // if the bin index is larger than the max bin index, force it to be the max bin index. It happen somehow...
    //   max_non_zero_x = hists[0]->GetNbinsX();

    // if( max_non_zero_x > 1e6 )
    //   max_non_zero_x = 8e5;
  ...
  std::pair < int, int > OptimizeRange( T* hist, int axis_param=0 )
  {
	...
	if( content > 0 )
	  {
	    bin_max = i;
	    std::cout << std::setw(10) << i << "\t" << content << std::endl;
	    break;
	  }
      }

    if( bin_max == axis->GetNbins() )
      bin_max = 1;
    
    std::cout << "detemination... " << bin_max << std::endl;
