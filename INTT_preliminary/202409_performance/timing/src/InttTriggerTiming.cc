#include "InttTriggerTiming.h"

using namespace std;

//____________________________________________________________________________..
InttTriggerTiming::InttTriggerTiming(const std::string &name) : 
  SubsysReco(name) 
{
...
//____________________________________________________________________________..
InttTriggerTiming::~InttTriggerTiming()
{
...
// private
/////////////////////////////////////////////////////////////////////////
int InttTriggerTiming::GetNodes(PHCompositeNode *topNode)
{
...
vector < std::pair < uint16_t, int > > InttTriggerTiming::GetBcoEventCounter()
{
  //cout << "vector < std::pair < uint16_t, int > > InttTriggerTiming::GetBcoEventCounter()" << endl;
...
std::vector < int > InttTriggerTiming::GetTriggerBits()
{
...
bool InttTriggerTiming::IsSame( InttRawHit* hit1, InttRawHit* hit2 )
{
...
vector < InttRawHit* > InttTriggerTiming::GetRawHits()
{
...
vector < InttRawHit* > InttTriggerTiming::GetRawHitsWithoutClone()
{
...
InttTriggerTiming::GetHits( TrkrHitSetContainer::ConstRange hitsets )
{
  /*!
...
void InttTriggerTiming::SetOutputDir( string dir )
{
  if( dir != "" )
    {
      output_dir_ = dir;
...
int InttTriggerTiming::Init(PHCompositeNode *topNode)
{
  return Fun4AllReturnCodes::EVENT_OK;
}

int InttTriggerTiming::InitRun(PHCompositeNode *topNode)
{
  ...
  hist_bco_diff_ = new TH1D( "bco_diff", "FPHX BCO;INTT Timing [BCO];Counts",
			     InttQa::kBco_max, 0, InttQa::kBco_max );

  hist_bco_diff_raw_ = new TH1D( "bco_diff_raw", "FPHX BCO (raw);INTT Timing [BCO];Counts",
				 120, 0, 120 );
...
int InttTriggerTiming::process_event(PHCompositeNode *topNode)
{
  ...
  for( int i=0; i<hits.size(); i++ )
    {
      ...
      int bco_diff = ( bco_fphx - (bco_intt & 0x7fU) + 128 ) % 128;

      hist_bco_diff_raw_->Fill( bco_diff );
      if( adc > 35 )
	{
	  hist_bco_diff_->Fill(  bco_diff );
...
int InttTriggerTiming::ResetEvent(PHCompositeNode *topNode)
{

  return Fun4AllReturnCodes::EVENT_OK;
}

int InttTriggerTiming::EndRun(const int runnumber)
{


  // Barrel correlation
  tf_output_->WriteTObject( hist_bco_diff_, hist_bco_diff_->GetName() );
  tf_output_->WriteTObject( hist_bco_diff_raw_, hist_bco_diff_raw_->GetName() );
...
int InttTriggerTiming::End(PHCompositeNode *topNode)
{

  return Fun4AllReturnCodes::EVENT_OK;
}

int InttTriggerTiming::Reset(PHCompositeNode *topNode)
{

  return Fun4AllReturnCodes::EVENT_OK;
}

void InttTriggerTiming::Print(const std::string &what) const
{

  int width = 100;
  cout << string( width, '-' ) << endl;
  cout << "InttTriggerTiming" << endl;
