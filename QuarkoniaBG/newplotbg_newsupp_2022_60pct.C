...
void newplotbg_newsupp_2022_60pct() {
...
double statscale = 1.4; // from 10B to 14B
...
int whichsuppression = 1; // 0=CMS suppression, 1=Strickland suppression

int npts1 = 10;
int npts2 = 8;
int npts3 = 7;
int npts3_rebin = 1;
if(whichsuppression==0) {npts3_rebin=4;}
...
}  else if(npts3_rebin==4) {
  xx3_rebin[0] = fUpsilonXPt->Integral(0.,2.)/fUpsilonPt->Integral(0.,2.);
  xx3_rebin[1] = fUpsilonXPt->Integral(2.,4.)/fUpsilonPt->Integral(2.,4.);
  xx3_rebin[2] = fUpsilonXPt->Integral(4.,6.)/fUpsilonPt->Integral(4.,6.);
  xx3_rebin[3] = fUpsilonXPt->Integral(6.,10.)/fUpsilonPt->Integral(6.,10.);
} else if(npts3_rebin==1) {
  xx3_rebin[0] = fUpsilonXPt->Integral(0.,10.)/fUpsilonPt->Integral(0.,10.);
} else {cerr << "ERROR: Wrong Y(3S) binning." << endl; return;}


// Raa for 0-60% centrality bin
double raa1s[nbins+1];
double raa2s[nbins+1];
double raa3s[nbins+1];
for(int i=0; i<nbins+1; i++) {raa1s[i]=0.654; raa2s[i]=0.277; raa3s[i]=0.075;}
if(whichsuppression==0) {for(int i=0; i<nbins+1; i++) {raa3s[i]=0.138;}}

TGraph* grRAA1S = new TGraph(nbins,xx1,raa1s);
TGraph* grRAA2S = new TGraph(nbins,xx2,raa2s);
TGraph* grRAA3S = new TGraph(nbins,xx3,raa3s);
...
//Npart for 0-60% centrality = 160.2
//Ncoll for 0-60% centrality = 388.8
//Ncoll for 0-10% centrality = 955.
//Npionpairs for 0-60% centrality = 0.406667
//Number of Upsilons in 0-60% centrality = 19405 1920 549
double combbg_corr_60pct = 0.406667 * 6.;
double corrbg_corr_60pct = 388.8/955. * 6.;

//---------- Use fixed numbers for 2022 estimate --------------------------------------
// Numbers from Marzia for 140 B MB Au+Au and 2400 B p+p events
// RAA for Y(3S) is 8.5% Number of Upsilons for 0-60% centrality bin
  int Nups1   = 19405;
  int Nups2   = 1920;
  int Nups3   = 549; // with CMS suppression
  if(whichsuppression==1) {Nups3   = 549*(3.5/8.5);} // with Strickland's suppression
  int Nups23 = Nups2+Nups3;
  int Nups1pp = 2.86e+03;
  int Nups2pp = 7.16e+02;
  int Nups3pp = 3.98e+02;
  int Nups23pp = Nups2pp+Nups3pp;
  ...
  hhdy[i]->Fit("expo","rql","",statscale_lowlim,statscale_uplim);
    ...
    for(int k=1; k<=hhcorrbg[i]->GetNbinsX(); k++) {
      if(hhcorrbg[i]->GetBinLowEdge(k)<statscale_lowlim || (hhcorrbg[i]->GetBinLowEdge(k)+hhcorrbg[i]->GetBinWidth(k))>statscale_uplim) {
        ...
        double tmp = corrbg_corr_60pct * statscale * hhcorrbg[i]->GetFunction("expo")->Eval(hhcorrbg[i]->GetBinCenter(k));
...
for(int i=0; i<nbins+1; i++) {
  ...
  for(int k=1; k<=hhbottom_pp[i]->GetNbinsX(); k++) {
    if(hhbottom_pp[i]->GetBinLowEdge(k)<statscale_lowlim || (hhbottom_pp[i]->GetBinLowEdge(k)+hhbottom_pp[i]->GetBinWidth(k))>statscale_uplim) {
      ...
      double tmp = corrbg_corr_60pct * ppcorr * fbottom_nosup_corr->Eval(hhbottom[i]->GetBinCenter(k)) * hhbottom[i]->GetFunction("expo")->Eval(hhbottom[i]->GetBinCenter(k));
      double tmprnd = myrandom->Poisson(tmp);
      if(tmprnd<0.) { tmprnd=0.; }
  ...
  hhcombbg[i]->Fit(fbg,"qrl","",statscale_lowlim,statscale_uplim);
    ...
    for(int k=1; k<=hhcombbg[i]->GetNbinsX(); k++) {
      if(hhcombbg[i]->GetBinLowEdge(k)<statscale_lowlim || (hhcombbg[i]->GetBinLowEdge(k)+hhcombbg[i]->GetBinWidth(k))>statscale_uplim) {
        ...
        double tmp = combbg_corr_60pct * statscale * hhcombbg[i]->GetFunction("fbg")->Eval(hhcombbg[i]->GetBinCenter(k));
        double tmprnd = myrandom->Poisson(tmp);
        if(tmprnd<0.) { tmprnd=0.; }
...
raapt_rebin3[2] = 5.;
raapt_rebin3[3] = 7.;
raa3_rebin[0] = grRAA3S->Eval(raapt_rebin2[0]);
raa3_rebin[1] = grRAA3S->Eval(raapt_rebin2[1]);
raa3_rebin[2] = grRAA3S->Eval(raapt_rebin2[2]);
raa3_rebin[3] = grRAA3S->Eval(raapt_rebin2[3]);
sum3_rebin[0] = truesum3[0]+truesum3[1];
sum3_rebin[1] = truesum3[2]+truesum3[3];
sum3_rebin[2] = truesum3[4]+truesum3[5];
sum3_rebin[3] = truesum3[6]+truesum3[7]+truesum3[8]+truesum3[9];
ersum3_rebin[0] = ersum3[0]+ersum3[1];
ersum3_rebin[1] = ersum3[2]+ersum3[3];
ersum3_rebin[2] = ersum3[4]+ersum3[5];
ersum3_rebin[3] = ersum3[6]+ersum3[7]+ersum3[8]+ersum3[9];
sum3pp_rebin[0] = sumpp3[0]+sumpp3[1];
sum3pp_rebin[1] = sumpp3[2]+sumpp3[3];
sum3pp_rebin[2] = sumpp3[4]+sumpp3[5];
sum3pp_rebin[3] = sumpp3[6]+sumpp3[7]+sumpp3[8]+sumpp3[9];
ersumpp3_rebin[0] = ersumpp3[0]+ersumpp3[1];
ersumpp3_rebin[1] = ersumpp3[2]+ersumpp3[3];
ersumpp3_rebin[2] = ersumpp3[4]+ersumpp3[5];
ersumpp3_rebin[3] = ersumpp3[6]+ersumpp3[7]+ersumpp3[8]+ersumpp3[9];
  ...
  erraa3_rebin[2] = raa3[2]*sqrt(ersum3_rebin[2]/sum3_rebin[2]/sum3_rebin[2] + ersumpp3_rebin[2]/sum3pp_rebin[2]/sum3pp_rebin[2]);
  erraa3_rebin[3] = raa3[3]*sqrt(ersum3_rebin[3]/sum3_rebin[3]/sum3_rebin[3] + ersumpp3_rebin[3]/sum3pp_rebin[3]/sum3pp_rebin[3]);
}
else if(npts3_rebin==2) {
// Rebin Y(3S) by 4
raapt_rebin3[0] = 2.;
raapt_rebin3[1] = 6.;
...
erraa3_rebin[0] = raa3[0]*sqrt(ersum3_rebin[0]/sum3_rebin[0]/sum3_rebin[0] + ersumpp3_rebin[0]/sum3pp_rebin[0]/sum3pp_rebin[0]);
} else {cerr << "ERROR: Wring Y(3S) binning." << endl; return;}
...
TCanvas* craa = new TCanvas("craa","R_{AA}",120,120,850,600);
...
//--- 3S state -------------------
TGraphErrors* gr3_rebin;
if(npts3_rebin==4) {
 gr3_rebin = new TGraphErrors(npts3_rebin-1,xx3_rebin,raa3_rebin,0,erraa3_rebin);
} else {
 gr3_rebin = new TGraphErrors(npts3_rebin,xx3_rebin,raa3_rebin,0,erraa3_rebin);
}
...
TLegend *leg = new TLegend(0.72,0.70,0.88,0.88);
leg->SetBorderSize(0);
leg->SetFillColor(10);
...
TLatex* l1 = new TLatex(0.5,1.02,"#font[72]{sPHENIX} Projection"); l1->SetTextSize(0.045); l1->SetTextFont(42); l1->Draw();
TLatex* l11 = new TLatex(0.5,0.93,"0-60% cent. Au+Au, Years 1-3"); l11->SetTextSize(0.045); l11->SetTextFont(42); l11->Draw();
TLatex* l2 = new TLatex(0.5,0.84,"21 nb^{-1} rec. Au+Au, 62 pb^{-1} samp. #it{p+p}");    l2->SetTextSize(0.045); l2->SetTextFont(42); l2->Draw();
TLatex* l3 = new TLatex(1.8,0.48,"STAR Prelim. 0-60% Y(2S+3S)");  l3->SetTextColor(kGreen+2);   l3->SetTextSize(0.045); l3->SetTextFont(42); l3->Draw();
TMarker* m1 = new TMarker(1.55,0.50,29); m1->SetMarkerSize(2); m1->SetMarkerColor(kGreen+2); m1->Draw();
...
double star_x[3],star_erx[3],star[3],star_stater[3],star_syser[3];
star_x[0] = 1.;
star_x[1] = 3.;
star_x[2] = 7.;
star_erx[0] = 0.16;
...
TGraphErrors* gr_star = new TGraphErrors(3,star_x,star,0,star_stater);
gr_star->SetMarkerStyle(kFullStar);
gr_star->SetMarkerColor(kGreen+2);
gr_star->SetLineColor(kGreen+2);
gr_star->SetLineWidth(2);
gr_star->SetMarkerSize(2);
gr_star->SetName("gr_star");

TGraphErrors* grsys_star = new TGraphErrors(3,star_x,star,star_erx,star_syser);
grsys_star->SetMarkerStyle(kFullStar);
grsys_star->SetMarkerColor(kGreen+2);
grsys_star->SetLineColor(kGreen+2);
grsys_star->SetLineWidth(2);
grsys_star->SetMarkerSize(2);
grsys_star->SetName("gr_star");
grsys_star->Draw("5");
gr_star->Draw("p");
 gr3_rebin->Draw("p");
