...
int likelihood_topology()
{
  ...
  float Et_miss_phi,Et_miss,tau_phi;
  float is_lq;

  // Get variables from tree //
  t->SetBranchAddress("Et_miss_phi",&Et_miss_phi);
  t->SetBranchAddress("Et_miss",&Et_miss);
  t->SetBranchAddress("reco_tau_phi",&tau_phi);
  ...
  for( int k = 0; k < n_tot; k++){
    ...
    x_bin = h_tau[0]->GetXaxis()->FindBin(Et_miss);
    likelihood.push_back( h_tau[0]->GetBinContent(x_bin) / (h_tau[0]->GetBinContent(x_bin) + h_NC[0]->GetBinContent(x_bin)));

    x_bin = h_tau[1]->GetXaxis()->FindBin(delta_phi);
    likelihood.push_back( h_tau[1]->GetBinContent(x_bin) / (h_tau[1]->GetBinContent(x_bin) + h_NC[1]->GetBinContent(x_bin)));
