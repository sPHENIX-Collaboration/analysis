...
makePlot_reco_electron_id()
{
  ...
  TCut cut_eta( "-1*log(tan(em_cluster_theta/2.0)) < 1" );
  ...
  h_e_over_p_pion->Draw("");
  h_e_over_p_electron->Draw("sames");
  ...
  TH1F* h_em_prob_electron = new TH1F("h_em_prob_electron", ";electron shape probability", 50, 0, 1);
  ...
  TH1F* h_em_eta_electron = new TH1F("h_em_eta_electron", ";#eta", 200, -4, 4);
