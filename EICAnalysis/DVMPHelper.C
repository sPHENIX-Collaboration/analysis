...
std::vector<float>
DVMPHelper::calculateInvariantMass_7()
{
  std::vector<float> inv_mass;
  ...
  for(unsigned i = 0; i < idx_electron.size() ; i++)
    {
      // Test if positron has eta in EEMC (eta < -1.434), if so, set ptotal to cluster_e
      if(rparticles[idx_positron].eta < -1.434)
	{
	  rparticles[idx_positron].ptotal = rparticles[idx_positron].e;
	}

      // Do an identical test for the electron
      if(rparticles[idx_electron.at(i)].eta < -1.434)
	{
	  rparticles[idx_electron.at(i)].ptotal = rparticles[idx_electron.at(i)].e;
	}
...
std::vector<float>
DVMPHelper::calculateInvariantMass_8()
{
  std::vector<float> inv_mass;
...
std::vector<float>
DVMPHelper::calculateInvariantMass_9()
{
  std::vector<float> inv_mass;
