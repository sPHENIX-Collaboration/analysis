...
void CEMC_Clusters_Full(const string &calib_path)
{
    ...
    {
        clusterCorrection->Get_eclus_CalibrationParameters().ReadFromFile("CEMC_RECALIB", "xml", 0, 0, calib_path);
                //raw location
                // string(getenv("CALIBRATIONROOT")) + string("/") + calib_path);
        clusterCorrection->Get_ecore_CalibrationParameters().ReadFromFile("CEMC_ECORE_RECALIB", "xml", 0, 0, calib_path);
                //raw location
                // string(getenv("CALIBRATIONROOT")) + string("/") + calib_path);
    }
