...
void CaloEvaluator::fillOutputNtuples(PHCompositeNode* topNode)
{
  ...
  if (_do_cluster_eval)
  {
    ...
    std::string clusternode_calib = "CLUSTER_POS_COR_" + _caloname;
    RawClusterContainer* clusters_calib = findNode::getClass<RawClusterContainer>(topNode, clusternode_calib.c_str());
    ...
    for (auto iterator = (clusters->getClustersMap()).begin(), iterator_calib = (clusters_calib->getClustersMap()).begin();
         iterator != (clusters->getClustersMap()).end() && iterator_calib != (clusters_calib->getClustersMap()).end();
         ++iterator, ++iterator_calib)
    {
