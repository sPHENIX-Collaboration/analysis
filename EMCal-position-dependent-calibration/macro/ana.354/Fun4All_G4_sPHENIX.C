    ...
    const int skip = 0)
{
  ...
  if (Input::SIMPLE)
  {
    ...
    // generation along the bottom boundary of the sector
    // INPUTGENERATOR::SimpleEventGenerator[0]->set_eta_range(0, 1.152);
    // INPUTGENERATOR::SimpleEventGenerator[0]->set_phi_range(-M_PI/32.0 - 0.02, -0.073 + 0.02);

    // generation along the top boundary of the sector
    // INPUTGENERATOR::SimpleEventGenerator[0]->set_eta_range(0, 1.152);
    // INPUTGENERATOR::SimpleEventGenerator[0]->set_phi_range(0.073 - 0.02, M_PI/32.0 + 0.02);
