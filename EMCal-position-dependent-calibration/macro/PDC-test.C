...
void myAnalysis::process_event(Long64_t start, Long64_t end) {
    ...
    for (Long64_t i = start; i <= end; ++i) {
        ...
        Float_t ecore_calib = ecore/h2Calib->GetBinContent(binx+1, biny+1);
        ...
        if(binx >= 0 && binx < bins_eta && biny >= 0 && biny < bins_phi) {
            hResponses[binx][biny]->Fill(response_calib);
            ++ctr;
        }
    }

    cout << "ecore min: " << ecore_min << ", ecore max: " << ecore_max << endl;
    cout << "ecore_calib min: " << ecore_calib_min << ", ecore_calib max: " << ecore_calib_max << endl;
    ...
    cout << "response_calib min: " << response_calib_min << ", response_calib max: " << response_calib_max << endl;
