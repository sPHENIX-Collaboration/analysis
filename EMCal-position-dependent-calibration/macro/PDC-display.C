...
void myAnalysis::process() {
    ...
    h2Response      = new TH2F("h2Response", "Gaussian Fit E_{Core}/E_{Truth}; p_{T}^{Truth} [GeV]; #eta_{Truth}", bins_pt, low_pt, high_pt, bins_eta, low_eta, high_eta);
    h2ResponseCalib = new TH2F("h2ResponseCalib", "Gaussian Fit Calib E_{Core}/E_{Truth}; p_{T}^{Truth} [GeV]; #eta_{Truth}", bins_pt, low_pt, high_pt, bins_eta, low_eta, high_eta);

    hResponseEta.push_back(new TH1F("hResponseEta_0", "Gaussian Fit E_{Core}/E_{Truth}, 3 GeV #leq p_{T}^{Truth} < 4 GeV; #eta_{Truth}; E_{Core}/E_{Truth}", bins_eta, low_eta, high_eta));
    hResponseCalibEta.push_back(new TH1F("hResponseCalibEta_0", "Gaussian Fit E_{Core}/E_{Truth}, 3 GeV #leq p_{T}^{Truth} < 4 GeV; #eta_{Truth}; E_{Core}/E_{Truth}", bins_eta, low_eta, high_eta));
    ...
    for(UInt_t i = 10; i < 15; ++i) {
       ...
       hResponseEta.push_back(new TH1F(name.c_str(), title.c_str(), bins_eta, low_eta, high_eta));
       hResponseCalibEta.push_back(new TH1F(name2.c_str(), title.c_str(), bins_eta, low_eta, high_eta));
    }

    stringstream s;
    for(Int_t i = 0; i < bins_eta; ++i) {
        s.str("");
       string name   = "hResponsePt_" + to_string(i);
       string name2  = "hResponseCalibPt_" + to_string(i);
       s << "Gaussian Fit E_{Core}/E_{Truth}, " << -1+i*0.2 << " #leq #eta_{Truth} < " << -1+(i+1)*0.2 << "; p_{T}^{Truth} [GeV]; E_{Core}/E_{Truth}";
       hResponsePt.push_back(new TH1F(name.c_str(), s.str().c_str(), bins_pt, low_pt, high_pt));
       hResponseCalibPt.push_back(new TH1F(name2.c_str(), s.str().c_str(), bins_pt, low_pt, high_pt));
    }

    cout << "hResponseEta: " << hResponseEta.size() << ", hResponseCalibEta: " << hResponseCalibEta.size() << endl;
    cout << "hResponsePt: " << hResponsePt.size() << ", hResponseCalibPt: " << hResponseCalibPt.size() << endl;
    ...
    for(UInt_t i = 0; i < bins_pt; ++i) {
        ...
        for(UInt_t j = 0; j < bins_eta; ++j) {
            ...
            if(fitStatus) {
                ...
                if(pt == 3 /*3 to 4 GeV bin*/) {
                    hResponseEta[0]->SetBinContent(j+1, mu);
                    hResponseEta[0]->SetBinError(j+1, mu_error);
                }

                hResponsePt[j]->SetBinContent(i+1, mu);
                hResponsePt[j]->SetBinError(i+1, mu_error);
            }
            ...
            if(fitStatus) {
                ...
                if(pt >= 10 && pt < 15) {
                    hResponseEta[pt-9]->SetBinContent(j+1, mu);
                    hResponseEta[pt-9]->SetBinError(j+1, mu_error);
                }
            ...
            if(fitStatus) {
                ...
                hResponseCalibPt[j]->SetBinContent(i+1, mu);
                hResponseCalibPt[j]->SetBinError(i+1, mu_error);
            }
    ...
    for(UInt_t i = 0; i < bins_eta; ++i) {
        legend = new TLegend(0.7,0.78,0.94,0.9);
        hResponsePt[i]->SetStats(0);
        hResponsePt[i]->SetLineColor(kBlue);
        hResponsePt[i]->GetYaxis()->SetRangeUser(0.98,1.07);
        legend->AddEntry(hResponsePt[i],"Uncalibrated", "PE");
        hResponsePt[i]->Draw("PE");

        hResponseCalibPt[i]->SetStats(0);
        hResponseCalibPt[i]->SetLineColor(kRed);
        legend->AddEntry(hResponseCalibPt[i],"Calibrated", "PE");
        hResponseCalibPt[i]->Draw("same PE");

        legend->SetTextSize(0.05);
        legend->Draw("same");

        c2->Print((output3).c_str(), "pdf portrait");
        c2->Print((outDir + "/hResponsePt_" + to_string(i) + ".png").c_str());
    }
