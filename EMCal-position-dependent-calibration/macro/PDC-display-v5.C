...
// input: /sphenix/user/anarde/output/validation-original-plots-0.3-25-GeV-v2.root
// output: /sphenix/user/anarde/output/1-31-24/original
// OR
// input: /sphenix/user/anarde/output/validation-residual-plots-0.3-25-GeV-v2.root
// output: /sphenix/user/anarde/output/1-31-24/nocalib
//
...
void myAnalysis::process() {
    ...
    if(plot_type == 1) {
        ...
        inputFile =  "output/validation-original-plots-0.3-25-GeV-v2.root";
        ...
        outDir    = "output/1-31-24/original";
        inputFile = "output/validation-original-plots-0.3-25-GeV-v2.root";
    }
    else {
        bins_response = 20;
        ...
        outDir    = "output/1-31-24/residual";
    ...
    if(plot_type == 1) {
        // original
        hResponseCalib  = new TH1F("hResponse", "Gaussian Fit E/E_{Truth}; E/E_{Truth}; Counts", bins_response, low_response, high_response);
        h2ResponseCalib = new TH2F("h2Response", "Gaussian Fit E/E_{Truth}; E_{Truth} [GeV]; Cluster #eta", bins_ge, low_ge, high_ge, bins_eta, low_eta, high_eta);
    }
    if(plot_type >= 2) {
        // calib
        hResponseCalib  = new TH1F("hResponseCalib", "Gaussian Fit E/E_{Truth}; E/E_{Truth}; Counts", bins_response, low_response, high_response);
        h2ResponseCalib = new TH2F("h2ResponseCalib", "Gaussian Fit Calib E/E_{Truth}; E_{Truth} [GeV]; Cluster #eta", bins_ge, low_ge, high_ge, bins_eta, low_eta, high_eta);
    }
    ...
    for(UInt_t i = 0; i < bins_ge; ++i) {
        for(UInt_t j = 2; j < bins_eta; ++j) {
            // string name = (plot_type == 1) ? "response_ecore/hResponseECore_"+to_string(i)+"_"+to_string(j):
            //                                  "response_ecore_calib/hResponseECoreCalib_"+to_string(i)+"_"+to_string(j);
            string name = (plot_type == 1) ? "response_e/hResponseE_"+to_string(i)+"_"+to_string(j):
                                             "response_e_calib/hResponseECalib_"+to_string(i)+"_"+to_string(j);
    ...
    auto h1 = (TH1F*)input->Get("hClusECalib");

    h1->Draw("HIST");

    c1->Print((output2).c_str(), "pdf portrait");
    c1->Print((outDir + "/qa-ClusterECalib.png").c_str());
    ...
    c1->Print((output3).c_str(), "pdf portrait");
    c1->Print((outDir + "/hResponseCalib.png").c_str());
