...
// input: /sphenix/u/anarde/scratch/data/validation-20-21-GeV.root
// output: /sphenix/user/anarde/output/validation-plots-20-21-GeV.root
//
// c++ includes --
#include <iostream>
#include <strings.h>
...
namespace myAnalysis {
    ...
    // UInt_t  bins_geta = 96;
    // Float_t low_geta  = -1.152;
    // Float_t high_geta = 1.152;

    UInt_t  bins_geta = 40;
    Float_t low_geta  = -1;
    Float_t high_geta = 1;

    // First 8 towers in eta are uninstrumented
    Float_t eta_bdry = -0.96;
}
...
void myAnalysis::init_hists() {
    ...
    // Float_t eta_min = -1;
    // Float_t eta_bin = 0.2;
    Float_t bin_width_geta = (high_geta-low_geta)/bins_geta;
    Float_t bin_width_p = (high_p-low_p)/bins_p;
    stringstream title;
    for(Int_t i = 0; i < bins_p; ++i) {
        vector<TH1F*> hDummy1;
        vector<TH1F*> hDummy2;
        // photon eta bins: 10 [-1,1]
        for(Int_t j = 0; j < bins_geta; ++j) {
            title.str("");
            title << "Response: " << low_p+i*bin_width_p << " #leq p^{Truth} < " << low_p+(i+1)*bin_width_p
                  << ", " << low_geta + j*bin_width_geta << " #leq #eta_{Truth} < " << low_geta + (j+1)*bin_width_geta
            ...
            title.str("");
            title << "Response Calib: " << low_p+i*bin_width_p << " #leq p^{Truth} < " << low_p+(i+1)*bin_width_p
                  << ", " << low_geta + j*bin_width_geta << " #leq #eta_{Truth} < " << low_geta + (j+1)*bin_width_geta
                  << "; E_{Core}/E_{Truth}; Counts";
...
void myAnalysis::process_event(UInt_t events) {
    ...
    auto hDummyPt  = new TH1F("hDummyPt","",bins_p,low_p,high_p);
...
void PDC_Analysis_v2(const string &i_input,
...
int main(int argc, char* argv[]) {
    ...
    PDC_Analysis_v2(argv[1], outputFile, ntp_cluster_entries);
