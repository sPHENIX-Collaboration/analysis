...
int CaloEmulatorTreeMaker::process_event(PHCompositeNode *topNode)
{
  ...
  _ll1out_trigger = findNode::getClass<LL1Out>(topNode, "LL1OUT_JET");

  _ll1out_raw_trigger = findNode::getClass<LL1Out>(topNode, "LL1OUT_RAW_JET");
  ...
  _trigger_primitives_raw_trigger = findNode::getClass<TriggerPrimitiveContainer>(topNode, "TRIGGERPRIMITIVES_RAW_JET");

  _trigger_primitives_trigger = findNode::getClass<TriggerPrimitiveContainer>(topNode, "TRIGGERPRIMITIVES_JET");
 ...
 if (_trigger_primitives_hcal_ll1)
  ...
  if (_ll1out_trigger)
      ...
      for (LL1Outv1::Iter iter = wrange.first ; iter != wrange.second ; ++iter)
	{
      ...
      b_trigger_bits.clear();
      for (unsigned int & it_b : *(_ll1out_trigger->GetTriggerBits()))
	{
	  b_trigger_bits.push_back(it_b);
	}
    }
  ...
  if (_ll1out_raw_trigger)
      ...
      for (LL1Outv1::Iter iter = wrange.first ; iter != wrange.second ; ++iter)
	{
      ...
      b_trigger_raw_bits.clear();
      for (unsigned int & it_b : *(_ll1out_raw_trigger->GetTriggerBits()))
	{
	  b_trigger_raw_bits.push_back(it_b);
	}
    }
