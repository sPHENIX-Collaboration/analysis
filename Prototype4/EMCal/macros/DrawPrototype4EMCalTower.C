...
void
DrawPrototype4EMCalTower( //
    const TString infile = "data/led_00000254.root_DSTReader.root", //
...
EMCDistribution_HCalCalibration()
{
  ...
  SaveCanvas(c1,
      TString(_file0->GetName()) + TString("_DrawPrototype4EMCalTower_")
  ...
  if (gain.Contains("CALIB"))
  ...
  else if (gain.Contains("RAW"))
    {
      if (full_gain)
        {
          h2 = new TH2F(hname,
              Form(";Tower Peak Amp (ADC);Count / bin"), 100,
