...
void plotQA_LevelingCorr()
{
  ...
  float fit_start = -0.8;
  float fit_stop  = 0.8;
  TCanvas *c_AsymmEnergy_leveling = new TCanvas("c_AsymmEnergy_leveling","c_AsymmEnergy_leveling",1500,500);
  c_AsymmEnergy_leveling->Divide(3,1);
  for(int i_pad = 0; i_pad < 3; ++i_pad)
  {
    c_AsymmEnergy_leveling->cd(i_pad+1);
    c_AsymmEnergy_leveling->cd(i_pad+1)->SetLeftMargin(0.15);
    c_AsymmEnergy_leveling->cd(i_pad+1)->SetBottomMargin(0.15);
    c_AsymmEnergy_leveling->cd(i_pad+1)->SetTicks(1,1);
    c_AsymmEnergy_leveling->cd(i_pad+1)->SetGrid(0,0);
  }

  c_AsymmEnergy_leveling->cd(1);
  ...
  cout  << "5  GeV: c_in = " << c_in_5GeV << ", c_out = " << c_out_5GeV << endl;
  cout  << "8  GeV: c_in = " << c_in_8GeV << ", c_out = " << c_out_8GeV << endl;
  cout  << "12 GeV: c_in = " << c_in_12GeV << ", c_out = " << c_out_12GeV << endl;
}
