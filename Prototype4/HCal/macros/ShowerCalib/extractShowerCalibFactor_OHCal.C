...
void extractShowerCalibFactor_OHCal()
{
  ...
  for(int i_pad = 0; i_pad < 3; ++i_pad)
  {
    ...
    h_mEnergyOut_pion[i_pad]->Draw("hE");
    ...
    f_gaus_Reco[i_pad]->SetRange(1.0,0.5*mean_energy[i_pad]);
    h_mEnergyOut_pion[i_pad]->Fit(f_gaus_Reco[i_pad],"NQR");
  ...
  const int i_energy = 1; // use 8 GeV for shower calibration

  const float showercalib = mean_energy[i_energy]/Reco_mean[i_energy];
  cout << "showercalib: " << mean_energy[i_energy] << "/" << Reco_mean[i_energy] << " = "  << showercalib << endl;
