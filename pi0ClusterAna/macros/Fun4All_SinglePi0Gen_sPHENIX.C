...
int Fun4All_SinglePi0Gen_sPHENIX(
    const int nEvents = 1,
    const int pid = 111,
    ...
    const string &outdir = ".")
{
  ...
  if (Input::SIMPLE)
  {
    ...
    INPUTGENERATOR::SimpleEventGenerator[0]->set_p_range(8.0, 8.0);
  }
  // Upsilons
...
//  G4MAGNET::magfield_rescale = 1.;  // make consistent with expected Babar field strength of 1.4T
  ...
  pi0ClusterAna *eval = new pi0ClusterAna("dummy", outputroot + "_pi0ClusterAna.root");
