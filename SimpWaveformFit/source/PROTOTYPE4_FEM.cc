...
bool PROTOTYPE4_FEM::SampleFit_PowerLawDoubleExp(  //
    ...
    double &peak_sample,                           //
    double &pedestal,                              //
    std::map<int, double> &parameters_io,double &chi2, const int verbosity)
{
  ...
  for (unsigned int iSample = 0; iSample < NSAMPLES - risetime * 3; iSample++)
  {
    if (abs(gpulse.GetY()[iSample] - pedestal) > abs(peakval - pedestal))
    {
      peakval = gpulse.GetY()[iSample];
  ...
  if (verbosity <= 1){
    TFitResultPtr fitres = gpulse.Fit(&fits, "QRN0W", "goff", 0., (double) NSAMPLES);
chi2 = fits.GetChisquare(); // get the chi2 or least squares value
    ...
    const int verbosity)
{
  ...
  double peakval = pedestal;
  for (unsigned int iSample = N_pedestal; iSample < NSAMPLES; iSample++)
  {
