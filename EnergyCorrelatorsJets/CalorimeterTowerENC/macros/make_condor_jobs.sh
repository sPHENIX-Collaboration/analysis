...
for i in $(seq 1 ${nFile}); do 
	...
	echo "Arguments       = ${truthj} ${caloclusterf} ${truthf} ${global} ${caloclusterf} ${g4hitf} ${i} ${nRuns}" >> $fname 
	echo "Output  	= /gpfs/mnt/gpfs02/sphenix/user/sgross/sphenix_analysis/EnergyCorrelatorsJets/CalorimeterTowerENC/macros/condor_files/condor_${i}.out " >> $fname
	echo "Error 		=/gpfs/mnt/gpfs02/sphenix/user/sgross/sphenix_analysis/EnergyCorrelatorsJets/CalorimeterTowerENC/macros/condor_files/condor_${i}.err " >> $fname
	echo "Log  		=/gpfs/mnt/gpfs02/sphenix/user/sgross/sphenix_analysis/EnergyCorrelatorsJets/CalorimeterTowerENC/macros/condor_files/condor_${i}.log" >> $fname
	echo "Initialdir  	= /gpfs/mnt/gpfs02/sphenix/user/sgross/sphenix_analysis/EnergyCorrelatorsJets/CalorimeterTowerENC/root_output" >> $fname
