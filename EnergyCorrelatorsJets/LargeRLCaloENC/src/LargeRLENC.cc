...
LargeRLENC::LargeRLENC(const int n_run/*=0*/, const int n_segment/*=0*/, const float jet_min_pT/*=0*/, const bool data/*=false*/,const std::string vari/*="E"*/, const std::string& name/* = "LargeRLENC"*/) 
{
	...
	n_evts=0;
	gInterpreter->GenerateDictionary("map<int, map<array<float,3>,float>>", "map;array");
	...
	if(n_segment%15 == 0 ){
		write_to_file=true;
		std::string text_out_filename="Missing_pT_for_felix_run-"+std::to_string(nRun)+"-"+std::to_string(nSegment)+".csv";
		text_out_file.open(text_out_filename.c_str());
