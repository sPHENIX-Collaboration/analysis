...
while ($runRange <= $nJobs)
    ...
    if( ! -d '/tmp/ahodges/logs/hotTower/log_hadd_'$runnumber) then 
	mkdir '/tmp/ahodges/logs/hotTower/log_hadd_'$runnumber
    endif 
    ...
    echo 'initialdir = /gpfs/mnt/gpfs02/sphenix/user/ahodges/macros_git/analysis/EMCal-commissioning/emcalHotTowerFinder/macro' >> submission_hadd/$runnumber/submitFor_Batch$runRange.job
    echo 'executable = /gpfs/mnt/gpfs02/sphenix/user/ahodges/macros_git/analysis/EMCal-commissioning/emcalHotTowerFinder/macro/hadd_condor.csh' >> submission_hadd/$runnumber/submitFor_Batch$runRange.job
    echo 'arguments = '$runRange $nFiles $runnumber >> submission_hadd/$runnumber/submitFor_Batch$runRange.job
    echo 'log  = /tmp/ahodges/logs/hotTower/log_hadd_'$runnumber'/job-'$runRange'.log' >> submission_hadd/$runnumber/submitFor_Batch$runRange.job
