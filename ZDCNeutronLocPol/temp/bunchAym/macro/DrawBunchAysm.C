...
void DrawBunchAysm(int runnumber = 42797)
{
//    std::string filename = "result_count_000" + std::to_string(runnumber) + "-"+std::to_string(cutvalue)+".root";
    ...
    crossingShift = 0;
    // Get the histograms
    TH1I *n_smd_v12 = dynamic_cast<TH1I *>(file->Get("h1_NSMD_v_12"));
    TH1I *n_smd_v67 = dynamic_cast<TH1I *>(file->Get("h1_NSMD_v_67"));
    TH1I *s_smd_v12 = dynamic_cast<TH1I *>(file->Get("h1_SSMD_v_12"));
    TH1I *s_smd_v67 = dynamic_cast<TH1I *>(file->Get("h1_SSMD_v_67"));
    if (!n_smd_v12 || !n_smd_v67 || !s_smd_v12 || !s_smd_v67)
    {
    ...
    for (int i = 1; i <= nBins; ++i)
    {
      ...
      double B = n_smd_v67->GetBinContent(i);
      double A_y = s_smd_v12->GetBinContent(i);
      double B_y = s_smd_v67->GetBinContent(i);
    ...
    graph_spinup_blue->SetTitle(Form("NSMD North Crossing Shift : %d;Bunch number;(L-R)/(L+R)",crossingShift));
    double x_min = 0;
    double x_max = 128;
    double y_min = -0.05;
  ...
  TF1 *fit1 = new TF1("fit1", "[0]", 0, 120);
    graph_spinup_blue->Fit(fit1, "R");

    TF1 *fit2 = new TF1("fit2", "[0]", 0, 120);
    graph_spindown_blue->Fit(fit2, "R");
...
TCanvas *canvas2 = new TCanvas("canvas_yellow", "South SMD", 800, 600);
    graph_spinup_yellow->SetTitle(Form("SSMD South Crossing Shift : %d;Bunch number;(L-R)/(L+R)",crossingShift));
    double x_min2 = 0;
    double x_max2 = 128;
    double y_min2 = -0.05;
    ...
    result.insert(pos, "-plot-crossing"+std::to_string(crossingShift));
