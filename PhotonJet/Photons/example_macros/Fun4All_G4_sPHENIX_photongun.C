int Fun4All_G4_sPHENIX_photongun(
		       const int nEvents = 3,
		       const int jobnum = 1,
		       const int photenergy = 10,
		       ...
		       const char * outputFile = "G4_garb.root",
		       ...
		       )
{
  ...
  if (readhits)
      ...
      else
	{
	  ...
	  //for perfect emcal sim
	  pgen->set_eta_range(0.324,0.324);
	  pgen->set_mom_range(photenergy,photenergy);
	  pgen->set_phi_range(1.00,1.00);

	  // pgen->set_eta_range(0.4,1);
	  //pgen->set_mom_range(2,16);
	  //pgen->set_phi_range(1.4,1.6);
