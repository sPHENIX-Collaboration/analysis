...
int Fun4All_G4_fsPHENIX_TruthJets(
		       const int nEvents = 10,
		       const int jobnum=1,
           const char * inputFile = "/sphenix/user/jdosbo/MC_files/xF_pT/1000inclusive_jets_510gev.dat",
		       const char * outputFile = "TruthJetGeneration.root",
...
//  se->Verbosity(0); // uncomment for batch production running with minimal output messages
  ...
  if (readhits)
  ...
  else if (runpythia8)
    {
      ...
      jettrig->SetEtaHighLow(-4.5,4.5);
      jettrig->SetMinJetPt(5);
      jettrig->SetJetR(0.5);
      jettrig->SetMinJetEnergy(5);
      jettrig->SetMinJetConstituents(3);
  ...
  if (do_fwd_jet_eval) Jet_FwdEval(outputFile);


  if(do_phot_jet_eval){
    gSystem->Load("libTruthPhotonJet.so");
    TruthPhotonJet *photjet = new TruthPhotonJet(outputFile);
    photjet->SetFirstEventNum(jobnum*nEvents);
    photjet->set_cluspt_mincut(1.);
    photjet->use_isocone_algorithm(0);
    photjet->set_jetcone_size(5);
    photjet->set_eta_lowhigh(-4.5,4.5);
    photjet->set_jetpt_mincut(2.);
