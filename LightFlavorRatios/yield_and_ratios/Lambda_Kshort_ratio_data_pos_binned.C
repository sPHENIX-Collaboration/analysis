...
void Lambda_Kshort_ratio_data_pos_binned()
{
  TFile* lambda_file = TFile::Open("/sphenix/user/mjpeters/analysis/LightFlavorRatios/mass_histograms/Lambda_data_pos.root");
  TFile* Ks_file = TFile::Open("/sphenix/user/mjpeters/analysis/LightFlavorRatios/mass_histograms/Kshort_data.root");
  ...
  CutSettings Lambda_cuts = StandardCuts::data_Lambda0_pos_cuts;
  ...
  TFile* fout = new TFile("fits_pos.root","RECREATE");

  ResonanceRatio analyzer(3122,"Lambda0","#Lambda^{0}",
                          310,"K_S0","K_{S}^{0}",
