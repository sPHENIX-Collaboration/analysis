...
rm output/truth_ratio_${SEGMENT}.root
rm output/truth_ratio_pos_${SEGMENT}.root
rm output/truth_ratio_neg_${SEGMENT}.root

# (Lambda + Lambdabar) / 2Ks
root -l -q -b calculate_truth_ratio.C\($LAMBDA_FLAVOR,$LAMBDA_DAUGHTERS,$KS_FLAVOR,$KS_DAUGHTERS,\"\#Lambda\",\"K^{0}_{S}\",\"$LAMBDA_FILEBASE\",\"$KS_FILEBASE\",0.5,\"output/truth_ratio\",${PROCESS}\)
# Lambda/Ks
root -l -q -b calculate_truth_ratio.C\($LAMBDA_FLAVOR,$LAMBDA_DAUGHTERS,$KS_FLAVOR,$KS_DAUGHTERS,\"\#Lambda\",\"K^{0}_{S}\",\"$LAMBDA_FILEBASE\",\"$KS_FILEBASE\",1.,\"output/truth_ratio_pos\",${PROCESS},false\)
#Lambdabar/Ks
root -l -q -b calculate_truth_ratio.C\($LAMBDABAR_FLAVOR,$LAMBDABAR_DAUGHTERS,$KS_FLAVOR,$KS_DAUGHTERS,\"\#Lambda\",\"K^{0}_{S}\",\"$LAMBDA_FILEBASE\",\"$KS_FILEBASE\",1.,\"output/truth_ratio_neg\",${PROCESS},false\)
