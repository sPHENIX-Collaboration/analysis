...
// Xi yield ratio (smeared / raw) vs pT — smearing systematic.
// Pre-filtered MC: no additional cuts applied.
// Run with:  root -l -b -q plot_filtered_xi_syst.C
...
void plot_filtered_xi_syst() {
    gStyle->SetOptStat(0); gStyle->SetOptTitle(0);

    const int N = 4;
    double avg_pt[]    = {0.9354, 1.1632, 1.3681, 1.7145};
    double ratio[]     = {1.019011, 0.962594, 1.015660, 1.001332};
    double ratio_err[] = {0.062541, 0.068638, 0.067675, 0.051657};
    double xerr[]      = {0.160, 0.090, 0.120, 0.255};
    double nr[]        = {526, 401, 447, 751};
    double ns[]        = {536, 386, 454, 752};
    double syst_pct[]  = {1.9011, 3.7406, 1.5660, 0.1332};

    // ── canvas 1: yield ratio ────────────────────────────────────────────────
    TCanvas *c1 = new TCanvas("c1","Xi yield ratio",900,650);
    ...
    gR->Draw("APE");
    gR->GetXaxis()->SetTitle("#Xi^{-} #it{p}_{T} (GeV/#it{c})");
    ...
    c1->SaveAs("plot_filtered_xi_syst_ratio.pdf");
    c1->SaveAs("plot_filtered_xi_syst_ratio.png");
    std::cout << "Saved plot_filtered_xi_syst_ratio\n";

    // ── canvas 2: raw vs smeared counts ─────────────────────────────────────
    TCanvas *c2 = new TCanvas("c2","Xi counts",900,650);
    ...
    mg->GetXaxis()->SetTitle("#Xi^{-} #it{p}_{T} (GeV/#it{c})");
    ...
    c2->SaveAs("plot_filtered_xi_syst_counts.pdf");
    c2->SaveAs("plot_filtered_xi_syst_counts.png");
    std::cout << "Saved plot_filtered_xi_syst_counts\n";

    // ── canvas 3: systematic % ───────────────────────────────────────────────
    TCanvas *c3 = new TCanvas("c3","Xi syst%",900,650);
    ...
    gS->Draw("APL");
    gS->GetXaxis()->SetTitle("#Xi^{-} #it{p}_{T} (GeV/#it{c})");
    ...
    c3->SaveAs("plot_filtered_xi_syst_pct.pdf");
    c3->SaveAs("plot_filtered_xi_syst_pct.png");
    std::cout << "Saved plot_filtered_xi_syst_pct\n";
}
