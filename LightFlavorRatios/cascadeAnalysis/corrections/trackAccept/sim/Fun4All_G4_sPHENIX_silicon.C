...
int Fun4All_G4_sPHENIX_silicon(
    const int nEvents = 100,
    const int process = 0,
    const std::string &outputFile = "G4sPHENIX_run79563.root",
    ...
    const std::string &outdir = ".")
{
  ...
  //Enable::MVTX_APPLYMISALIGNMENT = false;
  ...
  //std::string filename = std::format("/sphenix/lustre01/sphnxpro/mdc2/shijing_hepmc/OO_0_15fm/g4hits/run0035/G4Hits_sHijing_OO_0_15fm-0000000035-{:06}.root", process);
  //std::string filename = std::format("/sphenix/lustre01/sphnxpro/mdc2/js_pp200_signal/g4hits/run0029/detroit/G4Hits_pythia8_Detroit-0000000029-{:06}.root",process);
  //INPUTREADHITS::filename[0] = filename;
...
//G4TRACKING::convertSeedsContainerName = "SiliconTrackSeedContainer";
  //D0 generator
  //Input::DZERO = false;
  ...
  if (Input::SIMPLE)
  {
    ...
    else
    {
      INPUTGENERATOR::SimpleEventGenerator[0]->set_vertex_distribution_function(PHG4SimpleEventGenerator::Gaus,
                                                                                ...
                                                                                PHG4SimpleEventGenerator::Gaus);
      INPUTGENERATOR::SimpleEventGenerator[0]->set_vertex_distribution_mean(-0.072, 0.141, 0.);
    ...
    INPUTGENERATOR::SimpleEventGenerator[0]->set_pt_range(0.0, 3.);
...
//  G4MAGNET::magfield_rescale = 1.;  // make consistent with expected Babar field strength of 1.4T
  ...
  outputroot += "_proc_" + std::to_string(process);
  //if (Enable::TRACKING_EVAL) Tracking_Eval(outputroot + "_g4svtx_eval.root");

  SvtxEvaluator* eval;
  eval = new SvtxEvaluator("SVTXEVALUATOR", outputroot + "_g4svtx_eval.root", "SvtxTrackMap",
                           G4MVTX::n_maps_layer,
                           G4INTT::n_intt_layer,
                           G4TPC::n_gas_layer,
  ...
  std::string residstring = outputroot + "_resid.root";
