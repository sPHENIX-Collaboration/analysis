...
namespace HeavyFlavorReco
{
  ...
  bool run_KK_reco = true; // set to true if needed
  bool run_Kpi_reco = false; // set to true if needed

  std::string output_dir = "/sphenix/tg/tg01/hf/mjpeters/LightFlavorProduction/data/"; //Top dir of where the output nTuples will be written
  ...
  bool use_pid = true;
  bool use_local_PID_file = true;
  std::string local_PID_filename = "/sphenix/user/mjpeters/analysis/LightFlavorRatios/macros/data/dedx_82420.root";
...
void reconstruct_pipi_mass()
{
  ...
  kfparticle->useLocalPIDFile(use_local_PID_file);
  kfparticle->setLocalPIDFilename(local_PID_filename);
