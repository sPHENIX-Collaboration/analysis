#ifndef TRIVIALLAMBDAFEEDDOWNCORRECTION_H
#define TRIVIALLAMBDAFEEDDOWNCORRECTION_H
...
struct TrivialLambdaFeedDownCorrection : CorrectionHistogram1D
{
  std::shared_ptr<TFile> f;
  
  TrivialLambdaFeedDownCorrection(std::string filename, std::string hname)
  {
  ...
  TrivialLambdaFeedDownCorrection(const TrivialLambdaFeedDownCorrection& c)
  {
    f = c.f;
    h_corr = c.h_corr;
    name = c.name;
