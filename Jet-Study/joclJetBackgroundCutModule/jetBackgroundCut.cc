...
int jetBackgroundCut::process_event(PHCompositeNode *topNode)
{
  ...
  _rc->set_IntFlag("isDijet",isDijet);
  _rc->set_FloatFlag("frcem",frcem);
  _rc->set_FloatFlag("frcoh",frcoh);
  _rc->set_FloatFlag("maxJetET",maxJetET);
  _rc->set_FloatFlag("dPhi",dPhi);
